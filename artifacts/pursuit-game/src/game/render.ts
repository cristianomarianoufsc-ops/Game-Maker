...
import {
  ...
  DIVE_ENERGY_MAX, DIVE_ENERGY_COST,
} from './constants';
import { getPlatformCollisionRects } from './collision';

// ── Offscreen canvas reutilizável para o efeito de hurt (recorte por sprite) ──
let _hurtCanvas: HTMLCanvasElement | null = null;
let _hurtCtx: CanvasRenderingContext2D | null = null;
function _getHurtCtx(w: number, h: number): CanvasRenderingContext2D | null {
  if (typeof document === 'undefined') return null;
  if (!_hurtCanvas) {
    _hurtCanvas = document.createElement('canvas');
    _hurtCtx = _hurtCanvas.getContext('2d');
  }
  const cw = Math.ceil(w), ch = Math.ceil(h);
  if (_hurtCanvas.width !== cw) _hurtCanvas.width = cw;
  if (_hurtCanvas.height !== ch) _hurtCanvas.height = ch;
  return _hurtCtx;
}
...
type PlatType2 = { x: number; y: number; w: number; h: number; type: string; isRiverStump?: boolean; hideRender?: boolean; noGroup?: boolean };
...
function buildGroups(platforms: PlatType2[]): Group2[] {
  const elev = platforms
    .filter(p => p.type === 'platform' && !p.isRiverStump && !p.hideRender && !p.noGroup)
    .sort((a, b) => a.x - b.x);
  const groups: Group2[] = [];
...
export function drawPlatforms(
  ...
  for (let pi = 0; pi < platforms.length; pi++) {
    ...
    if (plat.type === 'wall') {
      // ── Placa de anúncio do prédio (wall x≈36144) ──────────────────────────
      if (plat.x > 36100 && plat.x < 36200) {
        const px = sx;
        const py = plat.y;
        const pw = plat.w;
        const ph = plat.h;

        // Armação metálica — braços horizontais saindo para a esquerda (prédio)
        const bracketCount = 4;
        const bracketSpacing = ph / (bracketCount + 1);
        for (let i = 1; i <= bracketCount; i++) {
          const by = py + Math.round(bracketSpacing * i) - 3;
          const bLen = 50;
          // Perfil do braço
          ctx.fillStyle = '#3e3e3e';
          ctx.fillRect(px - bLen, by, bLen, 7);
          // Facelete superior brilhante
          ctx.fillStyle = '#686868';
          ctx.fillRect(px - bLen, by, bLen, 2);
          // Sombra inferior
          ctx.fillStyle = 'rgba(0,0,0,0.50)';
          ctx.fillRect(px - bLen, by + 6, bLen, 2);
          // Parafuso na extremidade (parede do prédio — lado esquerdo)
          ctx.fillStyle = '#888';
          ctx.fillRect(px - bLen, by + 1, 5, 5);
          ctx.fillStyle = '#555';
          ctx.fillRect(px - bLen + 1, by + 2, 3, 3);
          // Parafuso na lateral da placa
          ctx.fillStyle = '#888';
          ctx.fillRect(px - 2, by + 1, 5, 5);
          ctx.fillStyle = '#555';
          ctx.fillRect(px - 1, by + 2, 3, 3);
        }

        // Moldura externa da placa (metal escuro fundido)
        ctx.fillStyle = '#1e1e1e';
        ctx.fillRect(px, py, pw, ph);
        // Borda esquerda reflexiva
        ...
        // Face interna da placa (área do anúncio)
        const fx = px + 3;
        const fy = py + 3;
        const fw = pw - 6;
        const fh = ph - 6;

        // Fundo azul-petróleo noturno
        ctx.fillStyle = '#0b1a2d';
        ctx.fillRect(fx, fy, fw, fh);

        // Faixa de título no topo (âmbar-laranja urbano)
        ctx.fillStyle = '#d98a10';
        ctx.fillRect(fx, fy + 6, fw, 6);
        ctx.fillStyle = '#f5b030';
        ctx.fillRect(fx, fy + 6, fw, 2); // brilho topo da faixa
        ctx.fillStyle = '#b06808';
        ctx.fillRect(fx, fy + 11, fw, 1);

        // Sub-faixa fina abaixo do título
        ctx.fillStyle = '#e8e8e8';
        ctx.fillRect(fx, fy + 14, fw, 1);
        ctx.fillRect(fx, fy + 16, fw * 0.6, 1);

        // Blocos de "texto" simulados (linhas brancas escalonadas)
        ctx.fillStyle = 'rgba(200,215,235,0.70)';
        const lineStart = fy + 26;
        const lineGap = 12;
        const totalLines = Math.floor((fh - 42) / lineGap);
        for (let li = 0; li < totalLines; li++) {
          const lw = li % 3 === 2 ? fw * 0.50 : fw * 0.80;
          ctx.fillRect(fx + 1, lineStart + li * lineGap, lw, 2);
        }

        // Rodapé — faixa vermelha (call-to-action)
        ctx.fillStyle = '#b02010';
        ctx.fillRect(fx, fy + fh - 18, fw, 15);
        ctx.fillStyle = '#e84030';
        ctx.fillRect(fx, fy + fh - 18, fw, 2); // brilho topo
        ctx.fillStyle = 'rgba(255,255,255,0.75)';
        ctx.fillRect(fx + 2, fy + fh - 13, fw - 4, 2);
        ctx.fillRect(fx + 2, fy + fh - 9, (fw - 4) * 0.55, 2);

        // Reflexo sutil na face (brilho de superfície envernizada)
        const gloss = ctx.createLinearGradient(fx, 0, fx + fw, 0);
        gloss.addColorStop(0, 'rgba(80,140,220,0.07)');
        gloss.addColorStop(0.45, 'rgba(80,140,220,0.14)');
        gloss.addColorStop(1, 'rgba(80,140,220,0.03)');
        ctx.fillStyle = gloss;
        ctx.fillRect(fx, fy, fw, fh);

        // Parafusos nos cantos da moldura externa
        ctx.fillStyle = '#555';
        for (const [bx, bby] of [[px + 1, py + 3], [px + 1, py + ph - 7]] as [number,number][]) {
          ctx.fillRect(bx, bby, 4, 4);
          ctx.fillStyle = '#888';
          ctx.fillRect(bx + 1, bby + 1, 2, 2);
          ctx.fillStyle = '#555';
        }
      ...
      // Corpo — concreto médio quente, legível contra tijolo escuro (#181614 dos prédios)
      ctx.fillStyle = '#4a3a2c';
      ctx.fillRect(sx, plat.y, plat.w, plat.h);

      // Fiadas horizontais de tijolos (textura)
      ctx.strokeStyle = 'rgba(0,0,0,0.40)';
      ctx.lineWidth = 1;
      for (let gy = plat.y + 9; gy < plat.y + plat.h; gy += 9) {
        ctx.beginPath();
        ctx.moveTo(sx, gy);
        ctx.lineTo(sx + plat.w, gy);
        ctx.stroke();
      }

      // Borda esquerda âmbar forte — indicador visual de "escalável"
      const lhGrad = ctx.createLinearGradient(sx, 0, sx + 7, 0);
      lhGrad.addColorStop(0, 'rgba(220,175,70,0.90)');
      lhGrad.addColorStop(1, 'rgba(220,175,70,0)');
      ctx.fillStyle = lhGrad;
      ctx.fillRect(sx, plat.y, 7, plat.h);

      // Sombra direita
      const rhGrad = ctx.createLinearGradient(sx + plat.w - 5, 0, sx + plat.w, 0);
      rhGrad.addColorStop(0, 'rgba(0,0,0,0)');
      rhGrad.addColorStop(1, 'rgba(0,0,0,0.55)');
      ctx.fillStyle = rhGrad;
      ctx.fillRect(sx + plat.w - 5, plat.y, 5, plat.h);

      // Grip marks douradas
      ctx.strokeStyle = 'rgba(220,180,80,0.30)';
      ctx.lineWidth = 1;
      for (let gy = plat.y + 12; gy < plat.y + plat.h - 6; gy += 18) {
        ctx.beginPath();
        ctx.moveTo(sx + 4, gy);
        ctx.lineTo(sx + plat.w - 4, gy);
        ctx.stroke();
      }
      ...
      if (!isRollUnder && plat.h <= 20) {
        ...
        // ── Janela (acima de plat.y) — omitida se noGroup:true ────
        if (!plat.noGroup) {
...
// ── KONG VAULT sprites ────────────────────────────────────────────────
// Pose 1: início do vault (borda do objeto) | Pose 2: no ar durante o vault.
// Ambas as imagens vêm com Horácio olhando para a ESQUERDA por padrão — espelha quando facingRight.
const KONG_VAULT_START_DISPLAY_H = 128;
const KONG_VAULT_AIR_DISPLAY_H = 158;
const KONG_VAULT_NATURAL_FACING_LEFT = true;

// ── Sprites presos ao jogador (escada, kong vault) — tamanho e espelhamento configuráveis no editor ──
type AttachedSpriteDisplay = { dw: number | null; dh: number | null; flip?: boolean; ox?: number; oy?: number };

/** Calcula a geometria de exibição do kong vault sem desenhar nada.
 *  Usado pelo editor para mostrar o ghost overlay quando o sprite está selecionado
 *  mas o jogador não está na animação ativa. */
export function computeKongVaultGeom(
  gs: GameState,
  phase: 'start' | 'air',
  img: HTMLImageElement,
  disp: AttachedSpriteDisplay | undefined,
): PlayerRenderGeom {
  const p = gs.player;
  const px = p.x - gs.camera.x;
  const py = p.y;
  const defaultDh = phase === 'start' ? KONG_VAULT_START_DISPLAY_H : KONG_VAULT_AIR_DISPLAY_H;
  const dh = disp?.dh ?? defaultDh;
  const dw = disp?.dw ?? Math.round(dh * (img.naturalWidth / img.naturalHeight));
  const anchorX = px + p.w / 2;
  const anchorY = py + PLAYER_H;
  const destX = anchorX - dw / 2 + (disp?.ox ?? 0);
  const destY = anchorY - dh + (disp?.oy ?? 0);
  let flip = KONG_VAULT_NATURAL_FACING_LEFT ? p.facingRight : !p.facingRight;
  if (disp?.flip) flip = !flip;
  return { poseKey: phase === 'start' ? 'kongVaultStart' : 'kongVaultAir', destX, destY, dw, dh, anchorX, flip };
}

// ── Todas as poses do Horácio editáveis individualmente (tamanho + espelhamento) ──
export type PlayerPoseKey =
  | 'idle' | 'run' | 'jump' | 'roll' | 'crouch' | 'divejump'
  | 'wallrun' | 'wallflip' | 'wallclimb' | 'sideflip'
  | 'ladder' | 'kongVaultStart' | 'kongVaultAir';

export type PoseDisplayOverrides = Partial<Record<PlayerPoseKey, AttachedSpriteDisplay>>;

export interface PlayerRenderGeom {
  poseKey: PlayerPoseKey;
  destX: number;
  destY: number;
  dw: number;
  dh: number;
...
export const PLAYER_POSE_LABELS: Record<PlayerPoseKey, string> = {
  idle: 'PARADO',
  run: 'CORRENDO',
  jump: 'PULO/QUEDA',
  roll: 'ROLAMENTO',
  ...
  wallflip: 'MORTAL NA PAREDE',
  wallclimb: 'SUBIDA DE PAREDE',
  sideflip: 'SIDE FLIP',
  ladder: 'ESCADA',
  kongVaultStart: 'KONG VAULT (INÍCIO)',
  kongVaultAir: 'KONG VAULT (AR)',
};
...
export function drawPlayer(
  ...
  kongVaultStartImg: HTMLImageElement | null = null,
  kongVaultAirImg: HTMLImageElement | null = null,
  poseOverrides?: PoseDisplayOverrides | null,
): PlayerRenderGeom | null {
  const playerLadderDisplay = poseOverrides?.ladder;
  const kongVaultStartDisplay = poseOverrides?.kongVaultStart;
  const kongVaultAirDisplay = poseOverrides?.kongVaultAir;
  const p = gs.player;
  const px = p.x - gs.camera.x;
  const py = p.y;
  ...
  if (p.state === 'climb' && p.touchingLadder) {
    ...
    if (sprite && sprite.complete && sprite.naturalWidth > 0) {
      ...
      const defaultDrawDh = Math.round(dh * 0.86);
      const defaultDw = Math.round(defaultDrawDh * (iw / ih));
      const drawDh = playerLadderDisplay?.dh ?? defaultDrawDh;
      const dw = playerLadderDisplay?.dw ?? defaultDw;
      const anchorX = px + p.w / 2;
      const destX = anchorX - dw / 2 + (playerLadderDisplay?.ox ?? 0);
      const destY = py + ph - drawDh + (playerLadderDisplay?.oy ?? 0); // ancora no pé, sobe só drawDh
      // Espelha apenas a subida — descida usa só o sprite frontal
      let mirror = !goingDown && Math.floor(Math.abs(p.y) / 28) % 2 === 1;
      if (playerLadderDisplay?.flip) mirror = !mirror;
      ctx.save();
      if (mirror) {
      ...
      ctx.drawImage(sprite, 0, 0, iw, ih, destX, destY, dw, drawDh);
      ctx.restore();
      return { poseKey: 'ladder', destX, destY, dw, dh: drawDh, anchorX, flip: mirror };
    }
  }

  // Kong Vault — pose de início (borda do objeto) e pose no ar
  if (p.kongVaultPhase === 'start' || p.kongVaultPhase === 'air') {
    const img = p.kongVaultPhase === 'start' ? kongVaultStartImg : kongVaultAirImg;
    if (img && img.complete && img.naturalWidth > 0) {
      // Obstáculos (ex: x:18972) usam display fixo na fase 'start'; carros/sprites usam poseOverrides normal.
      const OBSTACLE_VAULT_START: AttachedSpriteDisplay = { dw: 127, dh: 101, ox: 38, oy: -94, flip: true };
      const disp = p.kongVaultPhase === 'start'
        ? (p.kongVaultIsObstacle ? OBSTACLE_VAULT_START : kongVaultStartDisplay)
        : kongVaultAirDisplay;
      const defaultDh = p.kongVaultPhase === 'start' ? KONG_VAULT_START_DISPLAY_H : KONG_VAULT_AIR_DISPLAY_H;
      const iw = img.naturalWidth;
      const ih = img.naturalHeight;
      const dh = disp?.dh ?? defaultDh;
      const dw = disp?.dw ?? Math.round(dh * (iw / ih));
      const anchorX = px + p.w / 2;
      const anchorY = py + PLAYER_H;
      const destX = anchorX - dw / 2 + (disp?.ox ?? 0);
      const destY = anchorY - dh + (disp?.oy ?? 0);
      ...
      ctx.drawImage(img, 0, 0, iw, ih, destX, destY, dw, dh);
      ctx.restore();
      return { poseKey: p.kongVaultPhase === 'start' ? 'kongVaultStart' : 'kongVaultAir', destX, destY, dw, dh, anchorX, flip };
    }
  }

  // Blink when invincible
  if (p.invincible && Math.floor(gs.time / 80) % 2 === 0) return null;

  // Side flip animation
  if (p.isSideFlipping && sideFlipSheetImg && sideFlipSheetImg.complete && sideFlipSheetImg.naturalWidth > 0) {
    const ov = poseOverrides?.sideflip;
    const FRAME_COUNT = 5;
    ...
    const srcW = frameW - 2;
    const defaultDisplayH = 200;
    const displayH = ov?.dh ?? defaultDisplayH;
    const displayW = ov?.dw ?? Math.round(displayH * (srcW / frameH));
    const anchorX = px + p.w / 2;
    const anchorY = py + PLAYER_H / 2 - 8;
    const destX = anchorX - displayW / 2 + (ov?.ox ?? 0);
    const destY = anchorY - displayH / 2 + (ov?.oy ?? 0);
    let flip = !p.facingRight;
    if (ov?.flip) flip = !flip;
    ...
    ctx.restore();
    return { poseKey: 'sideflip', destX, destY, dw: displayW, dh: displayH, anchorX, flip };
  }

  // Usa o sprite de subida enquanto estiver no muro (subindo ou pendurado).
  // Quando ambos são false (saiu do muro), o bloco é ignorado e o sprite de pulo entra normalmente.
  if (p.state === 'wallclimb' && (p.isWallClimbUp || p.isWallHanging) && subidaSheetImg && subidaSheetImg.complete && subidaSheetImg.naturalWidth > 0) {
    const frameW = subidaSheetImg.naturalWidth / WALL_CLIMB_SHEET.frameCount;
    const frameH = subidaSheetImg.naturalHeight;
    const ovWC = poseOverrides?.wallclimb;
    // Mostra o frame de "pronto para saltar" (149×210) nos últimos ~30% da subida,
    // antes mesmo da física entrar em isWallHanging, para garantir que aparece na sequência.
    const climbT = p.wallClimbAdjustedDuration > 0
      ? Math.max(0, Math.min(1, 1 - p.wallClimbTimer / p.wallClimbAdjustedDuration))
      : 0;
    // Muro x:12100 usa comportamento antigo (só frame 0 durante a subida)
    const isLegacyWall = p.wallX === 12100;
    const showHangFrame = p.isWallHanging || (!isLegacyWall && climbT >= 0.10);
    const frame = showHangFrame ? WALL_CLIMB_SHEET.frameCount - 2 : 0;
    const frameScale = frame === 0 ? 1.05 : frame === 1 ? 0.90 : 1;
    const baseDh = showHangFrame ? WALL_CLIMB_SHEET.displayHHang : WALL_CLIMB_SHEET.displayH;
    const defaultDh = baseDh * frameScale;
    const dh = ovWC?.dh ?? defaultDh;
    const dw = ovWC?.dw ?? Math.round(dh * (frameW / frameH));
    const anchorX = px + p.w / 2;
    ...
    const destX = anchorX - dw / 2 + firstFrameWallOffset + wallSideNudge + (ovWC?.ox ?? 0);
    const rawDestY = anchorY - dh + (ovWC?.oy ?? 0);
    ...
    const destY = (frame === 0 && !p.isWallHanging)
      ...
      : rawDestY;
    let wcFlip = !p.facingRight;
    if (ovWC?.flip) wcFlip = !wcFlip;

    ctx.save();
    ctx.globalAlpha = 1; // garante opacidade total independente do estado anterior do canvas
    if (wcFlip) {
      ctx.translate(anchorX, 0);
      ctx.scale(-1, 1);
      ctx.translate(-anchorX, 0);
    ...
    ctx.restore();
    return { poseKey: 'wallclimb', destX, destY, dw, dh, anchorX, flip: wcFlip };
  }

  if (p.state === 'wallflip' && mortalSheetImg && mortalSheetImg.complete && mortalSheetImg.naturalWidth > 0) {
    const ovWF = poseOverrides?.wallflip;
    ...
    const dh = ovWF?.dh ?? WALL_FLIP_SHEET.displayH;
    const dw = ovWF?.dw ?? Math.round(dh * (frameW / frameH));
    const anchorX = px + p.w / 2;
    const anchorY = py + ph + WALL_FLIP_SHEET.offsetY;
    const destX = anchorX - dw / 2 + (ovWF?.ox ?? 0);
    const destY = anchorY - dh + (ovWF?.oy ?? 0);
    let wfFlip = !p.facingRight;
    if (ovWF?.flip) wfFlip = !wfFlip;
    ...
    ctx.restore();
    return { poseKey: 'wallflip', destX, destY, dw, dh, anchorX, flip: wfFlip };
  }
  ...
  if (isWallRunVisual) {
    if (!wallRunSheetImg || !wallRunSheetImg.complete || wallRunSheetImg.naturalWidth <= 0) return null;

    const ovWR = poseOverrides?.wallrun;
    ...
    const dh = ovWR?.dh ?? WALL_RUN_SHEET.displayH;
    const dw = ovWR?.dw ?? Math.round(dh * (frameW / frameH));
    const anchorX = px + p.w / 2;
    const anchorY = py + ph + 22;
    const destX = anchorX - dw / 2 + WALL_RUN_SHEET.offsetX + (ovWR?.ox ?? 0);
    const destY = anchorY - dh + (ovWR?.oy ?? 0);

    ctx.save();
    let shouldFlip = p.wallSide === 'left';
    if (ovWR?.flip) shouldFlip = !shouldFlip;
    if (shouldFlip) {
      ctx.translate(anchorX, 0);
      ctx.scale(-1, 1);
      ctx.translate(-anchorX, 0);
    ...
    ctx.restore();
    return { poseKey: 'wallrun', destX, destY, dw, dh, anchorX, flip: shouldFlip };
  }

  // Dive jump animation
  if (p.state === 'divejump' && diveSheetImg && diveSheetImg.complete && diveSheetImg.naturalWidth > 0) {
    const ovDJ = poseOverrides?.divejump;
    ...
    const dh = ovDJ?.dh ?? DIVE_SHEET.displayH;
    const dw = ovDJ?.dw ?? DIVE_SHEET.displayW;
    const anchorX = px + p.w / 2;
    const anchorY = py + PLAYER_H / 2 - 30;
    const destX = anchorX - dw / 2 + (ovDJ?.ox ?? 0);
    const destY = anchorY - dh / 2 + (ovDJ?.oy ?? 0);
    let djFlip = !p.facingRight;
    if (ovDJ?.flip) djFlip = !djFlip;
    ...
    return { poseKey: 'divejump', destX, destY, dw, dh, anchorX, flip: djFlip };
  }
  ...
  if (!p.autoRoll && (p.landingCrouch || p.isCrouching) && rollSheetImg && rollSheetImg.complete && rollSheetImg.naturalWidth > 0) {
    const ovCr = poseOverrides?.crouch;
    const dh = ovCr?.dh ?? ROLL_SHEET.displayH;
    const dw = ovCr?.dw ?? ROLL_SHEET.displayW;
    ...
    const destX = anchorX - dw / 2 + (ovCr?.ox ?? 0);
    const destY = anchorY - dh + (ovCr?.oy ?? 0);
    let crFlip = !p.facingRight;
    if (ovCr?.flip) crFlip = !crFlip;
    ctx.save();
  ...
  if (p.autoRoll && p.isRolling && rollSheetImg && rollSheetImg.complete && rollSheetImg.naturalWidth > 0) {
    ...
    const dh = ovRoll?.dh ?? ROLL_SHEET.displayH;
    const dw = ovRoll?.dw ?? ROLL_SHEET.displayW;
    ...
    const destX = anchorX - dw / 2 + (ovRoll?.ox ?? 0);
    const destY = anchorY - dh + frameDropY + (ovRoll?.oy ?? 0);
    let rollFlip = !p.facingRight;
    if (ovRoll?.flip) rollFlip = !rollFlip;
    ...
    return { poseKey: 'roll', destX, destY, dw, dh, anchorX, flip: rollFlip };
  }
  ...
  if (spriteImg && spriteImg.complete && spriteImg.naturalWidth > 0) {
    ...
    const isJumpAnim = (p.state === 'jump' || p.state === 'fall' || p.state === 'hurt' || p.state === 'dead' ||
                       (p.state === 'wallclimb' && !p.isWallClimbUp && !p.isWallHanging)) &&
                       jumpSheetImg && jumpSheetImg.complete && jumpSheetImg.naturalWidth > 0;

    const FOOT_OFFSET = 28;

    // ── Pose ativa neste grupo (para overrides de tamanho/espelhamento) ────
    const mainPoseKey: PlayerPoseKey = isRunAnim ? 'run' : isIdleAnim ? 'idle' : 'jump';
    const ovMain = !p.isRolling ? poseOverrides?.[mainPoseKey] : undefined;

    // ── Choose dimensions based on sprite source ──────────────────────────
    let defaultDh: number;
    if (p.isRolling) {
      defaultDh = SPRITE_DISPLAY_H * 0.55;
    } else if (isRunAnim) {
      defaultDh = RUN_DISPLAY_H;
    } else if (isIdleAnim) {
      defaultDh = SPRITE_DISPLAY_H;
    } else if (isJumpAnim) {
      defaultDh = JUMP_SHEET.displayH;
    } else {
      defaultDh = JUMP_DISPLAY_H;
    }
    const defaultDw = isRunAnim
      ? RUN_SHEET.displayW
      : isIdleAnim
    ...
    const dh = ovMain?.dh ?? defaultDh;
    const dw = ovMain?.dw ?? defaultDw;
    ...
    if (p.state === 'jump') {
      ...
      // Achata verticalmente o sprite de subida de parede (escada já retornou antes)
      scaleY = 0.78;
      scaleX = 1.06; // leve alargamento compensatório para não ficar estranho
    }

    const destX = anchorX - dw / 2 + (ovMain?.ox ?? 0);
    const destY = anchorY - dh + (ovMain?.oy ?? 0);
    ...
    let shouldFlip = !isStaticFacing && (naturallyFacingLeft ? p.facingRight : !p.facingRight);
    if (ovMain?.flip) shouldFlip = !shouldFlip;
    ...
    // Overlay vermelho-vinho recortado pelo contorno do sprite (sem bordas quadradas)
    if (p.state === 'hurt' || p.invincible) {
      const oc = _getHurtCtx(dw, dh);
      if (oc && _hurtCanvas) {
        const cw = Math.ceil(dw), ch = Math.ceil(dh);
        oc.clearRect(0, 0, cw, ch);
        // Re-desenha o sprite no offscreen para obter a máscara de pixels opacos
        if (isRunAnim && runSheetImg) {
          const frame = p.animFrame % RUN_SHEET.frameCount;
          oc.drawImage(runSheetImg, frame * RUN_SHEET.frameW, 0, RUN_SHEET.frameW, RUN_SHEET.frameH, 0, 0, dw, dh);
        } else if (isJumpAnim && jumpSheetImg) {
          const frame = p.vy < 0 ? 0 : 1;
          oc.drawImage(jumpSheetImg, JUMP_SHEET.frameSrcX[frame], 0, JUMP_SHEET.frameW, JUMP_SHEET.frameH, 0, 0, dw, dh);
        } else if (isIdleAnim && idleImg) {
          oc.drawImage(idleImg, 0, 0, IDLE_SPRITE.w, IDLE_SPRITE.h, 0, 0, dw, dh);
        } else if (spriteImg) {
          const reg = SPRITE_REGIONS[key];
          oc.drawImage(spriteImg, reg.sx, reg.sy, reg.sw, reg.sh, 0, 0, dw, dh);
        }
        // Pinta vermelho-vinho apenas onde há pixels do sprite (respeita transparência)
        oc.globalCompositeOperation = 'source-atop';
        oc.fillStyle = 'rgba(255,0,0,0.28)';
        oc.fillRect(0, 0, cw, ch);
        oc.globalCompositeOperation = 'source-over';
        // Blita o resultado de volta ao canvas principal (transforms de flip/lean já aplicados)
        ctx.drawImage(_hurtCanvas, destX, destY, dw, dh);
      }
    }
    ...
    const geomPoseKey: PlayerPoseKey = p.isRolling ? 'roll' : mainPoseKey;
    return { poseKey: geomPoseKey, destX, destY, dw, dh, anchorX, flip: shouldFlip };
  } else {
...
// --- Horácio sprite info panel (editor overlay) ---

export function drawHoracioSpriteInfo(
  ctx: CanvasRenderingContext2D,
  ...
  // Detecta qual sprite está ativo com base no estado atual do jogador
  type SpriteRow = { label: string; file: string; active: boolean };
  const isClimbing  = p.state === 'climb';
  const climbingUp  = isClimbing && p.vy <= 0;
  const rows: SpriteRow[] = [
    { label: 'estático',      file: 'horacio_transparent.png',        active: p.state === 'idle' && Math.abs(p.vx) < 0.5 },
    { label: 'idle animado',  file: 'idle_transparent.png',           active: p.state === 'idle' && Math.abs(p.vx) < 0.5 },
    { label: 'corrida',       file: 'run_sheet_transparent.png',      active: p.state === 'run' },
    { label: 'rolamento',     file: 'roll_sheet.png',                 active: p.state === 'roll' },
    { label: 'pulo / queda',  file: 'jump_sheet.png',                 active: p.state === 'jump' || p.state === 'fall' },
    { label: 'dive jump',     file: 'dive_jump_sheet.png',            active: p.isDivejumping },
    { label: 'wall run',      file: 'Wall_Run_1776005817769.png',     active: p.state === 'wallrun' },
    { label: 'wall flip',     file: 'mortal_1776009939272.png',       active: p.state === 'wallflip' },
    { label: 'subida parede', file: 'subida_1776012458574.png',       active: p.state === 'wallclimb' },
    { label: 'side flip',     file: 'SIDE_FLIP_1776053462942.png',    active: p.state === 'sideflip' },
    { label: 'escada sobe',   file: 'image_1776998047300.png',        active: isClimbing && climbingUp },
    { label: 'escada desce',  file: 'image_1776998292447.png',        active: isClimbing && !climbingUp },
  ];

  const activeRow = rows.find(r => r.active) ?? rows[0];

  // Contorno sempre visível ao redor do Horácio
  ctx.save();
  ctx.setLineDash([4, 3]);
  ctx.strokeStyle = isSelected ? 'rgba(80,255,160,1)' : 'rgba(80,255,160,0.45)';
  ctx.lineWidth   = isSelected ? 2 : 1;
  ctx.strokeRect(sx - 2, sy - 2, PLAYER_W + 4, PLAYER_H + 4);
  ctx.setLineDash([]);

  // Label sempre visível
  ctx.font      = 'bold 9px monospace';
  ctx.textAlign = 'center';
  ctx.fillStyle = isSelected ? 'rgba(80,255,160,1)' : 'rgba(80,255,160,0.7)';
  ctx.fillText('HORÁCIO', sx + PLAYER_W / 2, sy - 7);

  // Seta indicadora do sprite ativo (mesmo sem seleção, quando hover)
  if (!isSelected) {
    ctx.restore();
    return;
  }
  ...
  const PAD    = 8;
  const panelW = 308;
  // Header + active-banner + divider + list
  const HEADER_H  = 18;
  const BANNER_H  = 22;
  const DIVIDER_H = 6;
  const listH = rows.length * LINE_H;
  const panelH = PAD + HEADER_H + BANNER_H + DIVIDER_H + listH + PAD;

  // Posição: à direita ou à esquerda dependendo do espaço
  const rightX = sx + PLAYER_W + 12;
  const panelX = rightX + panelW > CANVAS_W ? sx - panelW - 12 : rightX;
  const panelY = Math.max(4, sy - 20);
  ...
  ctx.font       = 'bold 10px monospace';
  ctx.textAlign  = 'left';
  ctx.fillText('◙ HORÁCIO — sprites de animação', panelX + PAD, panelY + PAD + 9);

  // Banner do sprite ativo (em destaque amarelo)
  const bannerY = panelY + PAD + HEADER_H;
  ctx.fillStyle = 'rgba(255,220,60,0.18)';
  ctx.fillRect(panelX + PAD, bannerY, panelW - PAD * 2, BANNER_H);
  ctx.strokeStyle = 'rgba(255,220,60,0.7)';
  ctx.lineWidth = 1;
  ctx.strokeRect(panelX + PAD, bannerY, panelW - PAD * 2, BANNER_H);
  ctx.fillStyle  = 'rgba(255,230,80,1)';
  ctx.font       = 'bold 10px monospace';
  ctx.textAlign  = 'left';
  ctx.fillText('▶ ATIVO:', panelX + PAD + 4, bannerY + 8);
  ctx.fillStyle  = 'rgba(255,255,200,1)';
  ctx.font       = '9px monospace';
  ctx.fillText(activeRow.file, panelX + PAD + 56, bannerY + 8);
  ctx.fillStyle  = 'rgba(255,220,100,0.8)';
  ctx.font       = '8px monospace';
  ctx.fillText(`estado: ${p.state}`, panelX + PAD + 4, bannerY + 18);

  // Separador
  const listStartY = bannerY + BANNER_H + DIVIDER_H;
  ctx.strokeStyle = 'rgba(80,255,160,0.2)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(panelX + PAD, listStartY - 2);
  ctx.lineTo(panelX + panelW - PAD, listStartY - 2);
  ctx.stroke();

  // Lista completa — linha ativa destacada
  rows.forEach((row, i) => {
    const rowY = listStartY + i * LINE_H;
    if (row.active) {
      ctx.fillStyle = 'rgba(255,220,60,0.12)';
      ctx.fillRect(panelX + PAD, rowY - 1, panelW - PAD * 2, LINE_H);
    }
    // Label do estado
    ctx.fillStyle = row.active ? 'rgba(255,230,80,1)' : 'rgba(120,200,150,0.7)';
    ctx.font      = row.active ? 'bold 9px monospace' : '9px monospace';
    ctx.textAlign = 'left';
    ctx.fillText(row.label.padEnd(14), panelX + PAD + 2, rowY + 10);
    // Nome do arquivo
    ctx.fillStyle = row.active ? 'rgba(255,255,200,1)' : 'rgba(190,230,210,0.55)';
    ctx.font      = '9px monospace';
    ctx.fillText(row.file, panelX + PAD + 92, rowY + 10);
    // Seta para ativo
    if (row.active) {
      ctx.fillStyle = 'rgba(255,220,60,1)';
      ctx.fillText('◀', panelX + panelW - PAD - 8, rowY + 10);
    }
  });

  // Dica de uso no rodapé
  const footerY = panelY + panelH - 3;
  ctx.fillStyle  = 'rgba(80,255,160,0.4)';
  ctx.font       = '8px monospace';
  ctx.textAlign  = 'center';
  ctx.fillText('cole o nome do arquivo ativo no chat para referenciar este sprite', panelX + panelW / 2, footerY);

  ctx.restore();
}

// --- Bystander (figurante) info panel (editor overlay) ---

// ── Horácio volume panel (editor overlay) ────────────────────────────────────

export function getHoracioVolumePanelLayout(
  sx: number, sy: number, pw: number,
): { panelX: number; panelY: number; panelW: number; panelH: number; volTrackX: number; volTrackW: number; volTrackY: number } {
  const panelW = 200;
  const panelH = 62;
  const rightX = sx + pw + 12;
  const panelX = rightX + panelW > CANVAS_W ? sx - panelW - 12 : rightX;
  const panelY = Math.max(4, sy - 10);
  const volTrackX = panelX + 8;
  const volTrackW = panelW - 16;
  const volTrackY = panelY + 50;
  return { panelX, panelY, panelW, panelH, volTrackX, volTrackW, volTrackY };
}

export function drawHoracioVolumePanel(
  ctx: CanvasRenderingContext2D,
  vol: number,
  sx: number,
  sy: number,
  ...
  const layout = getHoracioVolumePanelLayout(sx, sy, pw);
  const { panelX, panelY, panelW, panelH, volTrackX, volTrackW, volTrackY } = layout;
  ctx.save();
  // Fundo
  ctx.fillStyle = 'rgba(6,12,20,0.95)';
  ctx.fillRect(panelX, panelY, panelW, panelH);
  ...
  ctx.fillStyle = 'rgba(80,255,160,1)';
  ctx.font = 'bold 10px monospace';
  ctx.fillText('◙ HORÁCIO — respiração', panelX + 8, panelY + 14);
  // Label de volume
  const v = Math.min(1, Math.max(0, vol));
  ctx.font = 'bold 9px monospace';
  ctx.fillStyle = 'rgba(180,255,220,0.95)';
  ctx.fillText(`🔊 VOL ${Math.round(v * 100)}%`, panelX + 8, panelY + 32);
  // Track background
  ctx.fillStyle = 'rgba(20,20,35,0.85)';
  ctx.strokeStyle = 'rgba(60,180,100,0.5)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.roundRect(volTrackX, volTrackY - 4, volTrackW, 6, 3);
  ctx.fill();
  ctx.stroke();
  // Fill
  const fillW = Math.max(4, volTrackW * v);
  ctx.fillStyle = 'rgba(80,255,160,0.92)';
  ctx.beginPath();
  ctx.roundRect(volTrackX, volTrackY - 4, fillW, 6, 3);
  ctx.fill();
  // Knob
  const knobX = volTrackX + fillW;
  ctx.beginPath();
  ctx.fillStyle = 'rgba(220,255,240,1)';
  ctx.strokeStyle = 'rgba(40,200,120,1)';
  ctx.lineWidth = 1.5;
  ctx.arc(knobX, volTrackY - 1, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.restore();
}

export function getBystanderPanelLayout(
  b: Bystander,
  displayW: number,
  camX: number,
): { panelX: number; panelY: number; panelW: number; panelH: number; volTrackX: number; volTrackW: number; volTrackY: number } {
  ...
  const sx = b.x - camX;
  const sy = GROUND_Y + NPC_FOOT_OFFSET - displayH;
  const panelW = 220;
  const panelH = 84;
  const rightX = sx + displayW + 10;
  const panelX = rightX + panelW > CANVAS_W ? sx - panelW - 10 : rightX;
  const panelY = Math.max(4, sy - 10);
  const volTrackX = panelX + 8;
  const volTrackW = panelW - 16;
  const volTrackY = panelY + 72;
  return { panelX, panelY, panelW, panelH, volTrackX, volTrackW, volTrackY };
}

export function drawBystanderInfo(
  ctx: CanvasRenderingContext2D,
  ...
  bystanders.forEach((b, idx) => {
    ...
    const sheet = b.spriteId === 1 ? sheet1 : b.spriteId === 2 ? sheet2 : b.spriteId === 3 ? sheet3 : sheet4;
    ...
    ctx.save();
    ctx.setLineDash([4, 3]);
    ctx.strokeStyle = isSelected ? 'rgba(255,160,80,1)' : 'rgba(255,160,80,0.35)';
    ctx.lineWidth = isSelected ? 2 : 1;
    ctx.strokeRect(sx - 2, sy - 2, displayW + 4, displayH + 4);
    ctx.setLineDash([]);

    ctx.font = 'bold 9px monospace';
    ctx.textAlign = 'center';
    ctx.fillStyle = isSelected ? 'rgba(255,160,80,1)' : 'rgba(255,160,80,0.65)';
    ctx.fillText(NAMES[b.spriteId], sx + displayW / 2, sy - 7);

    if (isSelected) {
      const layout = getBystanderPanelLayout(b, displayW, camX);
      const { panelX, panelY, panelW, panelH, volTrackX, volTrackW, volTrackY } = layout;
      ctx.fillStyle = 'rgba(6,12,20,0.95)';
      ctx.fillRect(panelX, panelY, panelW, panelH);
      ...
      ctx.font = 'bold 10px monospace';
      ctx.fillText(`◙ ${NAMES[b.spriteId]}`, panelX + 8, panelY + 14);
      ctx.font = '9px monospace';
      ctx.fillStyle = 'rgba(255,220,180,0.9)';
      ctx.fillText(`x: ${Math.round(b.x)}   y: ${Math.round(b.y)}`, panelX + 8, panelY + 29);
      ctx.fillText(`estado: ${b.state}   sprite: ${b.spriteId}`, panelX + 8, panelY + 42);
      ctx.fillStyle = 'rgba(255,180,100,0.5)';
      ctx.font = '8px monospace';
      ctx.fillText('clique fora para desselecionar', panelX + 8, panelY + 52);

      // Slider de volume individual do NPC
      const vol = b.sfxVolume ?? 1;
      ctx.font = 'bold 9px monospace';
      ctx.fillStyle = 'rgba(255,210,150,0.95)';
      ctx.fillText(`🔊 VOL ${Math.round(vol * 100)}%`, panelX + 8, panelY + 64);
      ctx.fillStyle = 'rgba(20,20,35,0.85)';
      ...
      ctx.fill();
      ctx.stroke();
      const fillW = Math.max(4, volTrackW * Math.min(1, Math.max(0, vol)));
      ...
      ctx.strokeStyle = 'rgba(255,140,30,1)';
      ctx.lineWidth = 1.5;
      ctx.arc(knobX, volTrackY - 1, 5, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
...
// --- Player pose editor handles (overlay do editor) — genérico para QUALQUER pose do Horácio ---

export function drawPlayerPoseEditorHandles(
  ctx: CanvasRenderingContext2D,
  geom: PlayerRenderGeom,
  display: AttachedSpriteDisplay | null | undefined,
  isSelected: boolean,
): void {
  if (!isSelected) return;

  const { destX, destY, dw, dh, anchorX } = geom;
  const label = PLAYER_POSE_LABELS[geom.poseKey];
  const HANDLE_S = 9;
  const color = '120,220,255';

  ctx.save();
  ctx.strokeStyle = `rgba(${color},0.95)`;
  ctx.lineWidth = 1.5;
  ctx.setLineDash([4, 3]);
  ctx.strokeRect(destX, destY, dw, dh);
  ctx.setLineDash([]);

  const ox = Math.round(display?.ox ?? 0);
  const oy = Math.round(display?.oy ?? 0);
  ctx.fillStyle = `rgba(${color},0.9)`;
  ctx.font = 'bold 9px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(`▣ ${label}  ${Math.round(dw)}×${Math.round(dh)}${display?.flip ? '  ↔' : ''}`, anchorX, destY - 14);
  ctx.fillText(`ox:${ox}  oy:${oy}  ↑↓←→ mover`, anchorX, destY - 5);
  ctx.textAlign = 'left';

  {
    // Handles de bordas (círculos — redimensionam só 1 eixo)
    const edgeHandles = [
      { hx: destX,          hy: destY + dh / 2 },
      { hx: destX + dw,     hy: destY + dh / 2 },
      { hx: anchorX,        hy: destY },
      { hx: anchorX,        hy: destY + dh },
    ];
    for (const h of edgeHandles) {
      ctx.beginPath();
      ctx.arc(h.hx, h.hy, HANDLE_S / 2, 0, Math.PI * 2);
    ...
    // Handles de cantos (losangos — escala uniforme, proporção travada)
    const cornerHandles = [
      { hx: destX + dw, hy: destY },
      { hx: destX,      hy: destY },
      { hx: destX + dw, hy: destY + dh },
      { hx: destX,      hy: destY + dh },
    ];
    const cs = HANDLE_S / 2 + 1; // raio do losango
    for (const h of cornerHandles) {
      ctx.beginPath();
      ctx.moveTo(h.hx,      h.hy - cs); // topo
      ctx.lineTo(h.hx + cs, h.hy);      // dir
      ctx.lineTo(h.hx,      h.hy + cs); // baixo
      ctx.lineTo(h.hx - cs, h.hy);      // esq
      ctx.closePath();
    ...
    // Handle de mover — centro do sprite (quadrado com ✛)
    const mhx = destX + dw / 2;
    const mhy = destY + dh / 2;
    const mhs = HANDLE_S + 3;
    ctx.fillStyle = 'rgba(255,220,80,0.95)';
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.roundRect(mhx - mhs / 2, mhy - mhs / 2, mhs, mhs, 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = '#333';
    ctx.font = `bold ${mhs - 2}px monospace`;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('✛', mhx, mhy);
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.roundRect(resetBtnX, resetBtnY, 52, 16, 3);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = `rgba(${color},0.9)`;
    ctx.font = 'bold 9px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('↺ RESET', anchorX, resetBtnY + 11);
    ctx.textAlign = 'left';

    const flipBtnX = anchorX + 30;
    const flipBtnY = resetBtnY;
    ctx.fillStyle = display?.flip ? `rgba(${color},0.35)` : 'rgba(30,30,50,0.88)';
    ctx.strokeStyle = `rgba(${color},0.7)`;
    ctx.beginPath();
    ctx.roundRect(flipBtnX, flipBtnY, 40, 16, 3);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = `rgba(${color},0.9)`;
    ctx.font = 'bold 9px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('↔', flipBtnX + 20, flipBtnY + 11);
    ctx.textAlign = 'left';
  }
  ctx.restore();
}
...
export function drawDogs(
  ...
  for (const dog of dogs) {
    ...
    if (dog.animState === 'idle' && dogIdle && dogIdle.complete && dogIdle.naturalWidth > 0) {
      ...
      // ── Breathing animation — ofegante/panting ──────────────────────────────
      // ~60 respirações/min (cão agitado/ofegante)
      const breathCycle = Date.now() * 0.0065;
      const breathPhase = Math.sin(breathCycle);

      // Patas ancoradas fixas no chão — só a parte de cima se move
      const baseY = GROUND_Y - displayH + 24;
      const idleScreenY = baseY; // sem bob no sprite inteiro

      // Parte de cima escala para cima a partir da cintura (±11% de expansão)
      const breathScale = 1 + breathPhase * 0.11;
      const waistRatio  = 0.50; // ponto de cintura do cachorro

      // Flip context for left-facing dog (same as original code)
      if (!dog.facingRight) {
        ctx.translate(screenX + idleDisplayW, 0);
        ctx.scale(-1, 1);
      }
      const drawX = dog.facingRight ? screenX : 0;

      // Pass 1: full sprite — paws stay at bobOffset position
      ctx.drawImage(dogIdle, 0, 0, idleSrcW, idleSrcH,
        drawX, idleScreenY, idleDisplayW, displayH);

      // Pass 2: upper body only, scaled upward from waist (chest expansion)
      const waistAbsY = idleScreenY + displayH * waistRatio;
      const topSrcH   = Math.round(idleSrcH * waistRatio);
      const topDestH  = displayH * waistRatio * breathScale;
      const topDestY  = waistAbsY - topDestH; // grow upward from waist

      ctx.save();
      ctx.beginPath();
      ctx.rect(drawX - 2, topDestY - 2, idleDisplayW + 4, topDestH + 4);
      ctx.clip();
      ctx.drawImage(dogIdle, 0, 0, idleSrcW, topSrcH,
        drawX, topDestY, idleDisplayW, topDestH);
      ctx.restore();
    } else {
...
export function drawParticles(
  ...
  for (const p of gs.particles) {
    ...
    ctx.beginPath();
    const _r = p.size * alpha;
    if (_r <= 0) continue;
    ctx.arc(p.x - gs.camera.x, p.y, _r, 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
...
export function drawHUD(
  ...
  ctx.fillRect(10, 10, 240, 66);
  ctx.strokeStyle = 'rgba(80,75,110,0.8)';
  ctx.lineWidth = 1;
  ctx.strokeRect(10, 10, 240, 66);
  ...
  ctx.fillStyle = COLORS.uiText;
  ctx.font = '11px monospace';
  ctx.fillText('VIDAS', 20, 46);
  for (let i = 0; i < 3; i++) {
    ctx.fillStyle = i < lives ? 'rgba(0,220,255,0.85)' : 'rgba(60,55,80,0.7)';
    ctx.fillRect(70 + i * 22, 34, 14, 14);
    ctx.strokeStyle = i < lives ? 'rgba(0,200,255,0.5)' : 'rgba(80,75,110,0.3)';
    ctx.lineWidth = 1;
    ctx.strokeRect(70 + i * 22, 34, 14, 14);
  }

  // Score + world X (for level editing reference)
  ctx.fillStyle = COLORS.uiTextBright;
  ctx.font = 'bold 13px monospace';
  ctx.fillText(`DIST: ${score}m  X:${Math.floor(p.x)}`, 20, 66);
  ...
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText('RUN', CANVAS_W - 207, 20);

  // Dive energy bar — same style as RUN, stacked right below it
  ctx.fillStyle = 'rgba(40,35,60,0.8)';
  ctx.fillRect(CANVAS_W - 210, 34, 200, 20);
  const diveEnergyPct = p.diveEnergy / DIVE_ENERGY_MAX;
  const diveFull  = diveEnergyPct >= 0.99;
  const diveReady = p.diveEnergy >= DIVE_ENERGY_COST - 0.5;
  const diveFill   = diveFull  ? 'rgba(60,200,80,0.60)'  : diveReady ? 'rgba(255,200,0,0.60)'  : 'rgba(255,60,60,0.45)';
  const diveStroke = diveFull  ? 'rgba(80,220,100,0.55)' : diveReady ? 'rgba(255,220,0,0.55)'  : 'rgba(255,80,80,0.5)';
  ctx.fillStyle = diveFill;
  ctx.fillRect(CANVAS_W - 210, 34, 200 * diveEnergyPct, 20);
  ctx.strokeStyle = diveStroke;
  ctx.lineWidth = 1;
  ctx.strokeRect(CANVAS_W - 210, 34, 200, 20);
  ctx.fillStyle = COLORS.uiText;
  ctx.font = '10px monospace';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'middle';
  ctx.fillText('DIVE ROLL', CANVAS_W - 207, 44);
  ctx.textBaseline = 'alphabetic';
...
export function getMenuHitAreas() {
  const cx = CANVAS_W / 2;
  const cy = CANVAS_H / 2;
  const storyBtnW = 200;
  const storyBtnH = 40;
...
export function drawMenuScreen(
  ...
  ctx.fill();
  ctx.strokeStyle = 'rgba(160,150,200,0.55)';
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.fillStyle = 'rgba(190,180,230,0.75)';
  ctx.font = '14px monospace';
  ctx.textAlign = 'center';
  ctx.fillText('⚙', gearCx, gearCy + 5);

  // Title
  ctx.fillStyle = '#d8d5e8';
  ctx.font = 'bold 26px monospace';
  ctx.fillText('O PREÇO DA ORDEM', cx, cy - 78);

  // Divider below title
  ctx.strokeStyle = 'rgba(80,75,110,0.4)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(cx - 160, cy - 58);
  ctx.lineTo(cx + 160, cy - 58);
  ctx.stroke();

  // ── Cores compartilhadas: focado = azul, desfocado = cinza ─────────────────
  const FOCUSED_BG      = 'rgba(0,200,255,0.16)';
  const FOCUSED_BORDER  = 'rgba(0,220,255,1.0)';
  const FOCUSED_TEXT    = 'rgba(0,230,255,1.0)';
  const FOCUSED_SUB     = 'rgba(0,200,240,0.65)';
  const UNFOCUSED_BG    = 'rgba(40,38,55,0.06)';
  const UNFOCUSED_BORDER= 'rgba(110,105,130,0.30)';
  const UNFOCUSED_TEXT  = 'rgba(130,127,145,0.55)';
  const UNFOCUSED_SUB   = 'rgba(100,97,118,0.35)';

  // MODO HISTÓRIA button  (focus 0)
  const storyFocused = menuFocus === 0;
  ctx.fillStyle = storyFocused ? FOCUSED_BG : UNFOCUSED_BG;
  ctx.fillRect(storyBtnX, storyBtnY, storyBtnW, storyBtnH);
  ctx.strokeStyle = storyFocused ? FOCUSED_BORDER : UNFOCUSED_BORDER;
  ctx.lineWidth = storyFocused ? 2 : 1;
  ctx.strokeRect(storyBtnX, storyBtnY, storyBtnW, storyBtnH);
  ctx.fillStyle = storyFocused ? FOCUSED_TEXT : UNFOCUSED_TEXT;
  ctx.font = storyFocused ? 'bold 16px monospace' : '13px monospace';
  ctx.fillText('MODO HISTÓRIA', cx, storyBtnY + (storyFocused ? 22 : 24));
  ctx.fillStyle = storyFocused ? FOCUSED_SUB : UNFOCUSED_SUB;
  ctx.font = '9px monospace';
  ...
  ctx.lineWidth = trainFocused ? 2 : 1;
  ctx.strokeRect(trainBtnX, trainBtnY, trainBtnW, trainBtnH);
  ctx.fillStyle = trainFocused ? FOCUSED_TEXT : UNFOCUSED_TEXT;
  ctx.font = trainFocused ? 'bold 14px monospace' : '11px monospace';
  ctx.fillText('SALA DE TREINO', cx, trainBtnY + (trainFocused ? 19 : 20));
  ctx.fillStyle = trainFocused ? FOCUSED_SUB : UNFOCUSED_SUB;
  ctx.font = '9px monospace';
  ...
  ctx.lineWidth = optFocused ? 2 : 1;
  ctx.strokeRect(optBtnX, optBtnY, optBtnW, optBtnH);
  ctx.fillStyle = optFocused ? FOCUSED_TEXT : UNFOCUSED_TEXT;
  ctx.font = optFocused ? 'bold 14px monospace' : '11px monospace';
  ctx.fillText('OPÇÕES', cx, optBtnY + (optFocused ? 16 : 17));
  ctx.fillStyle = optFocused ? FOCUSED_SUB : UNFOCUSED_SUB;
  ctx.font = '9px monospace';
  ctx.fillText('O', cx, optBtnY + (optFocused ? 26 : 25));

  // Botão mute — canto inferior-esquerdo do painel
  const { muteBtnX, muteBtnY, muteBtnW, muteBtnH } = getMenuHitAreas();
  ctx.fillStyle = menuMuted ? 'rgba(200,60,60,0.18)' : 'rgba(0,200,255,0.10)';
  ctx.fillRect(muteBtnX, muteBtnY, muteBtnW, muteBtnH);
  ctx.strokeStyle = menuMuted ? 'rgba(200,80,80,0.65)' : 'rgba(0,200,255,0.40)';
  ctx.lineWidth = 1;
  ctx.strokeRect(muteBtnX, muteBtnY, muteBtnW, muteBtnH);
  ctx.fillStyle = menuMuted ? 'rgba(220,100,100,0.90)' : 'rgba(0,210,255,0.75)';
  ctx.font = '9px monospace';
  ctx.textAlign = 'center';
  ctx.fillText(menuMuted ? '🔇 MUDO  M' : '🔊 SOM   M', muteBtnX + muteBtnW / 2, muteBtnY + 13);

  // Footer — author credit inside panel
  ctx.fillStyle = 'rgba(95,88,125,0.45)';
  ctx.font = '9px monospace';
  ctx.fillText('DE CRISTIANO MARIANO  ·  CAPÍTULO 1: LINHAS INVISÍVEIS', cx, panelY + panelH - 12);

  // Créditos musicais — obrigatório CC BY 4.0
  ctx.textAlign = 'right';
  ctx.fillStyle = 'rgba(100,95,130,0.5)';
  ctx.font = '8px monospace';
...
export function drawOptionsScreen(ctx: CanvasRenderingContext2D): void {
  const cx = CANVAS_W / 2;
  const cy = CANVAS_H / 2;
  ...
  const panelH = 240;
  const panelY = cy - 120;
  ctx.fillStyle = 'rgba(18,15,30,0.98)';
  ctx.fillRect(cx - 230, panelY, 460, panelH);
  ctx.strokeStyle = 'rgba(100,90,140,0.8)';
  ctx.lineWidth = 1;
  ctx.strokeRect(cx - 230, panelY, 460, panelH);
  // Red accent top
  ctx.fillStyle = 'rgba(255,60,60,0.65)';
  ctx.fillRect(cx - 230, panelY, 460, 3);

  ctx.textAlign = 'center';

  // Title
  ctx.fillStyle = 'rgba(200,195,230,0.9)';
  ctx.font = 'bold 15px monospace';
  ctx.fillText('OPÇÕES', cx, panelY + 28);

  // Divider
  ctx.strokeStyle = 'rgba(80,75,110,0.4)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.moveTo(cx - 180, panelY + 40);
  ctx.lineTo(cx + 180, panelY + 40);
  ctx.stroke();

  // Section label — CONTROLES
  ctx.fillStyle = 'rgba(150,145,175,0.65)';
  ctx.font = '9px monospace';
  ctx.fillText('CONTROLES', cx, panelY + 60);

  ctx.fillStyle = 'rgba(160,155,185,0.75)';
  ctx.font = '11px monospace';
  ctx.fillText('← →  CORRER   |   ESPAÇO / ↑  PULAR', cx, panelY + 82);
  ctx.fillText('SHIFT / Z  ROLAR   |   ↓ + ESPAÇO (correndo)  MERGULHO', cx, panelY + 100);
  ctx.fillText('PULE SUBINDO + PAREDE  =  CORRIDA NA PAREDE', cx, panelY + 118);

  // Divider
  ctx.strokeStyle = 'rgba(80,75,110,0.3)';
  ctx.beginPath();
  ctx.moveTo(cx - 180, panelY + 136);
  ctx.lineTo(cx + 180, panelY + 136);
  ctx.stroke();

  // Navigation hint
  ctx.fillStyle = 'rgba(110,105,140,0.65)';
  ctx.font = '9px monospace';
  ctx.fillText('ESPAÇO / ESC — FECHAR', cx, panelY + 156);

  // Divider before training button
  ctx.strokeStyle = 'rgba(80,75,110,0.3)';
  ctx.beginPath();
  ctx.moveTo(cx - 180, panelY + 170);
  ctx.lineTo(cx + 180, panelY + 170);
  ctx.stroke();

  // Training room button
  const btnY = panelY + 185;
  const btnH = 32;
  ctx.fillStyle = 'rgba(70,55,110,0.75)';
  ctx.fillRect(cx - 140, btnY, 280, btnH);
  ctx.strokeStyle = 'rgba(140,120,200,0.6)';
  ctx.lineWidth = 1;
  ctx.strokeRect(cx - 140, btnY, 280, btnH);
  // Left accent bar
  ctx.fillStyle = 'rgba(160,120,255,0.8)';
  ctx.fillRect(cx - 140, btnY, 3, btnH);

  ctx.fillStyle = 'rgba(210,195,255,0.95)';
  ctx.font = 'bold 12px monospace';
  ctx.fillText('[ T ]  SALA DE TREINO', cx, btnY + 13);
  ctx.fillStyle = 'rgba(150,140,185,0.65)';
  ctx.font = '9px monospace';
  ctx.fillText('pratique os movimentos livremente', cx, btnY + 26);

  ctx.textAlign = 'left';
}

// ── Sala de Treino — fundo e plataformas ────────────────────────────────────
export function drawTrainingRoom(
  ctx: CanvasRenderingContext2D,
  platforms: import('./types').Platform[],
  camX: number,
  playerX: number,
  camY: number = 0,
  completedFlags: { dive?: boolean; sideFlip?: boolean } = {},
): void {
  // Background: sala escura estilo concreto
  const grad = ctx.createLinearGradient(0, 0, 0, CANVAS_H);
  ...
  // Grid de fundo (referência visual)
  ctx.strokeStyle = 'rgba(60,55,85,0.25)';
  ctx.lineWidth = 1;
  const gridSize = 60;
  for (let gx = (-camX % gridSize + gridSize) % gridSize; gx < CANVAS_W; gx += gridSize) {
    ctx.beginPath(); ctx.moveTo(gx, 0); ctx.lineTo(gx, CANVAS_H); ctx.stroke();
  }
  for (let gy = 0; gy < CANVAS_H; gy += gridSize) {
    ctx.beginPath(); ctx.moveTo(0, gy); ctx.lineTo(CANVAS_W, gy); ctx.stroke();
  }

  // O chão, os objetos e as dicas ligadas ao mundo acompanham a câmera.
  // O balão do tic-tac é desenhado depois, preso ao espaço vazio da tela.
  ctx.save();
  ctx.translate(0, -camY);

  // Chão
  ctx.fillStyle = '#2e2b3e';
  ctx.fillRect(0, GROUND_Y, CANVAS_W, CANVAS_H - GROUND_Y);
  ctx.fillStyle = 'rgba(110,100,150,0.45)';
  ctx.fillRect(0, GROUND_Y, CANVAS_W, 2);

  // ── Helper: desenha balão de dica centralizado em screenX, topo em tipY ──────
  // Balão de dica — 3 camadas: nome do movimento / o que fazer / tecla
  const KEY_GREEN = 'rgba(100,220,160,0.95)';

  const drawHintBalloon = (
    screenX: number,
    tipY: number,
    moveName: string,   // ex: "PULO", "DIVE ROLL"
    action: string,     // ex: "pule por cima"
    keyHint: string,    // ex: "↓ + ESPAÇO"
    nameColor: string,
    actionColor: string,
    keyColor: string,
    subAction?: string,       // linha extra entre action e keyHint
    highlightLine?: string,   // linha destacada entre action e subAction
    highlightColor?: string,  // cor da linha destacada
  ) => {
    const has5 = !!(subAction && highlightLine);
    const boxW = subAction ? 340 : 270;
    const boxH = has5 ? 140 : subAction ? 122 : 84;
    const bx = screenX - boxW / 2;
    const by = tipY - boxH - 14;
    ctx.save();
    ctx.textAlign = 'center';
    ctx.fillStyle = 'rgba(20,16,38,0.88)';
    ctx.strokeStyle = 'rgba(130,108,210,0.65)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ...
    if (has5) {
      // 5 linhas: title / action / highlight / subAction / keyHint
      ctx.font = 'bold 16px monospace';
      ctx.fillStyle = nameColor;
      ctx.fillText(moveName,      screenX, by + 14 + 16);          // y=30
      ctx.font = '13px monospace';
      ctx.fillStyle = actionColor;
      ctx.fillText(action,        screenX, by + 30 + 7 + 13);      // y=50
      ctx.font = 'bold 14px monospace';
      ctx.fillStyle = highlightColor ?? nameColor;
      ctx.fillText(highlightLine!, screenX, by + 50 + 5 + 14);     // y=69
      ctx.font = '13px monospace';
      ctx.fillStyle = actionColor;
      ctx.fillText(subAction!,    screenX, by + 69 + 5 + 13);      // y=87
      ctx.font = 'bold 17px monospace';
      ctx.fillStyle = keyColor;
      ctx.fillText(keyHint,       screenX, by + 87 + 8 + 17);      // y=112
    } else if (subAction) {
      // 4 linhas
      ctx.font = 'bold 16px monospace';
      ctx.fillStyle = nameColor;
      ctx.fillText(moveName, screenX, by + 14 + 16);
      ctx.font = '13px monospace';
      ctx.fillStyle = actionColor;
      ctx.fillText(action,    screenX, by + 14 + 16 + 7 + 13);
      ctx.font = '13px monospace';
      ctx.fillStyle = actionColor;
      ctx.fillText(subAction, screenX, by + 14 + 16 + 7 + 13 + 6 + 13);
      ctx.font = 'bold 17px monospace';
      ctx.fillStyle = keyColor;
      ctx.fillText(keyHint,   screenX, by + 14 + 16 + 7 + 13 + 6 + 13 + 8 + 17);
    } else {
      // 3 linhas
      ctx.font = 'bold 14px monospace';
      ctx.fillStyle = nameColor;
      ctx.fillText(moveName, screenX, by + 10 + 14);
      ctx.font = '12px monospace';
      ctx.fillStyle = actionColor;
      ctx.fillText(action,   screenX, by + 10 + 14 + 5 + 12);
      ctx.font = 'bold 16px monospace';
      ctx.fillStyle = keyColor;
      ctx.fillText(keyHint,  screenX, by + 10 + 14 + 5 + 12 + 6 + 16);
    }
    // Seta ▼ abaixo da caixa
    ctx.font = '13px monospace';
    ctx.fillStyle = 'rgba(140,120,190,0.6)';
    ctx.fillText('▼', screenX, by + boxH + 11);
    ctx.restore();
  };

  // Helper: check verde permanente de stage concluído
  ...
  // Dica de dive roll — segue Horácio enquanto ele estiver entre a 1ª caixa e o muro baixo
  {
    const ZONE_START = 262;  // logo após a primeira caixa (x:200 + w:62)
    const ZONE_END   = 1880; // para antes de sobrepor o balão do wall run (x:2176 - margens)
    if (playerX > ZONE_START && playerX < ZONE_END) {
      const screenX = playerX - camX + PLAYER_W / 2;
      drawHintBalloon(
        screenX,
        GROUND_Y - 135,
        'DIVE ROLL',
  ...
  // Plataformas da sala
  let hasTicTacWall = false;
  let hasTrainingLadder = false;
  for (const p of platforms) {
    if (p.type === 'ground') continue;
    const sx = p.x - camX;
    if (sx + p.w < -20 || sx > CANVAS_W + 20) continue;

    if (p.type === 'wall') {
      if (p.isLadder) {
        hasTrainingLadder = true;
        // Escada final: dois trilhos, degraus regulares e uma faixa quente
        // no centro, seguindo o aspecto metálico da referência enviada.
        const ladderTop = p.y;
        const ladderBottom = p.y + p.h;
        ctx.fillStyle = 'rgba(130,88,40,0.32)';
        ctx.fillRect(sx + 5, ladderTop, p.w - 10, p.h);
        ctx.strokeStyle = '#34343a';
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.moveTo(sx + 5, ladderTop);
        ctx.lineTo(sx + 5, ladderBottom);
        ctx.moveTo(sx + p.w - 5, ladderTop);
        ctx.lineTo(sx + p.w - 5, ladderBottom);
        ctx.stroke();
        ctx.strokeStyle = 'rgba(210,190,150,0.72)';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(sx + 4, ladderTop);
        ctx.lineTo(sx + 4, ladderBottom);
        ctx.moveTo(sx + p.w - 4, ladderTop);
        ctx.lineTo(sx + p.w - 4, ladderBottom);
        ctx.stroke();
        ctx.strokeStyle = '#686870';
        ctx.lineWidth = 3;
        for (let ry = ladderTop + 10; ry < ladderBottom - 3; ry += 20) {
          ctx.beginPath();
          ctx.moveTo(sx + 5, ry);
          ctx.lineTo(sx + p.w - 5, ry);
          ctx.stroke();
        }
        ctx.strokeStyle = 'rgba(225,205,165,0.48)';
        ctx.lineWidth = 1;
        for (let ry = ladderTop + 10; ry < ladderBottom - 3; ry += 20) {
          ctx.beginPath();
          ctx.moveTo(sx + 6, ry - 1);
          ctx.lineTo(sx + p.w - 6, ry - 1);
          ctx.stroke();
        }
        // Flash verde de acerto
        if ((p.flashTimer ?? 0) > 0) {
          ctx.save();
          ctx.fillStyle = `rgba(60,255,100,${Math.min(0.55, (p.flashTimer! / 400) * 0.65).toFixed(2)})`;
          ctx.fillRect(sx + 5, ladderTop, p.w - 10, p.h);
          ctx.restore();
        }
        if (p.completed) drawCheckmark(sx + p.w / 2, ladderTop - 22);
      } else if (p.endWall) {
        // Grande barreira final: ocupa toda a altura útil e todo o lado direito.
        ctx.fillStyle = '#171521';
        ctx.fillRect(sx, p.y, p.w, p.h);
        ctx.fillStyle = 'rgba(190,55,75,0.75)';
        ctx.fillRect(sx, p.y, 4, p.h);
        ctx.fillStyle = 'rgba(110,45,75,0.42)';
        ctx.fillRect(sx + 4, p.y, 2, p.h);
        ctx.strokeStyle = 'rgba(80,65,105,0.32)';
        ctx.lineWidth = 1;
        for (let wy = p.y + 18; wy < p.y + p.h; wy += 22) {
          ctx.beginPath();
          ctx.moveTo(sx + 8, wy);
          ctx.lineTo(Math.min(sx + p.w, CANVAS_W + 30), wy);
          ctx.stroke();
        }
        ctx.save();
        ctx.textAlign = 'center';
        ctx.font = 'bold 13px monospace';
        ctx.fillStyle = 'rgba(235,105,120,0.8)';
        ctx.fillText('FIM DA ÁREA DE TREINO', Math.min(sx + 170, CANVAS_W - 170), Math.max(p.y + 48, camY + 48));
        ctx.font = '9px monospace';
        ctx.fillStyle = 'rgba(190,145,165,0.62)';
        ctx.fillText('RETORNE AO MENU COM ESC', Math.min(sx + 170, CANVAS_W - 170), Math.max(p.y + 64, camY + 64));
        ctx.restore();
      } else {
        // Muro: concreto escuro com bordas iluminadas
        ctx.fillStyle = '#252233';
        ctx.fillRect(sx, p.y, p.w, p.h);
        // Bordas laterais
        ctx.fillStyle = 'rgba(130,115,170,0.5)';
        ctx.fillRect(sx, p.y, 2, p.h);
        ctx.fillRect(sx + p.w - 2, p.y, 2, p.h);
        // Topo
        ctx.fillStyle = 'rgba(155,140,200,0.55)';
        ctx.fillRect(sx, p.y, p.w, 3);
        // Ranhuras horizontais (textura)
        ctx.strokeStyle = 'rgba(80,72,110,0.35)';
        ctx.lineWidth = 1;
        for (let wy = p.y + 20; wy < p.y + p.h; wy += 20) {
          ctx.beginPath(); ctx.moveTo(sx, wy); ctx.lineTo(sx + p.w, wy); ctx.stroke();
        }
        // Flash verde de acerto
        if ((p.flashTimer ?? 0) > 0) {
          ctx.save();
        ...
        if (p.completed) drawCheckmark(sx + p.w / 2, p.y - 22);
      }
      // Rótulo + dicas — o tic-tac substitui a dica genérica de wall run.
      if (p.isLadder) {
        // A dica da escada é desenhada fixa na viewport depois do mundo.
      } else if (p.tictacWall) {
        hasTicTacWall = true;
      } else if (p.noHint) {
        // Parede de fim de fase: sem instrução, apenas bloqueio visual/físico.
      } else if (p.climbable && !p.noHang) {
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'WALL RUN', 'pule subindo o muro com', '→ + ESPAÇO',
          'rgba(185,165,230,0.95)', 'rgba(145,130,180,0.85)', KEY_GREEN);
      } else if (p.noHang) {
        // A parede de saída pertence ao mesmo exercício; não cria uma
        // segunda dica genérica sobre o objeto paralelo.
      } else if (p.w > p.h) {
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'PULO', 'pule por cima', 'ESPAÇO',
          'rgba(210,235,255,0.95)', 'rgba(170,210,250,0.85)', KEY_GREEN);
      } else {
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'PULO', 'pule por cima', 'ESPAÇO',
          'rgba(210,188,255,0.95)', 'rgba(170,148,220,0.85)', KEY_GREEN);
      }

    } else if (p.type === 'platform') {
      if (p.isRollUnder) {
        // Bloco simples para treino de rolamento — mesmo estilo dos muros
        ctx.fillStyle = '#252233';
        ctx.fillRect(sx, p.y, p.w, p.h);
        ctx.fillStyle = 'rgba(130,115,170,0.5)';
        ctx.fillRect(sx, p.y, 2, p.h);
        ...
        if (p.completed) drawCheckmark(sx + p.w / 2, p.y - 22);
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'ROLAMENTO', 'role por baixo de objetos', '→ + ↓',
          'rgba(180,230,200,0.95)', 'rgba(140,195,165,0.88)', KEY_GREEN);
      } else {
        ctx.fillStyle = '#3a3555';
        ctx.fillRect(sx, p.y, p.w, p.h);
        ctx.fillStyle = 'rgba(150,140,190,0.45)';
        ctx.fillRect(sx, p.y, p.w, 2);
        ctx.save();
        ctx.textAlign = 'center';
        ctx.font = '8px monospace';
        ctx.fillStyle = 'rgba(130,122,165,0.6)';
        ctx.fillText('PLATAFORMA', sx + p.w / 2, p.y - 7);
        ctx.restore();
      }

    } else if (p.type === 'obstacle') {
      // Obstáculo (monkey vault ou kong vault) — mesma cor roxa
      ctx.fillStyle = '#2a2240';
      ctx.fillRect(sx, p.y, p.w, p.h);
      ctx.fillStyle = 'rgba(160,120,240,0.45)';
      ctx.fillRect(sx, p.y, p.w, 3);
      ctx.fillStyle = 'rgba(120,95,180,0.3)';
      ctx.fillRect(sx, p.y, 3, p.h);
      ...
      if (p.isKongVault) {
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'KONG VAULT', 'Mergulhe sobre o obstáculo', '↓ + ESPAÇO',
          'rgba(200,200,200,0.95)', 'rgba(160,165,170,0.88)', KEY_GREEN,
          'para alcançar maior distância',
          'com um DIVE ROLL',
          'rgba(255,220,60,0.95)');
      } else {
        drawHintBalloon(sx + p.w / 2, p.y - 10,
          'MONKEY', 'use as mãos no obstáculo', 'encoste no obstáculo + pulo',
          'rgba(195,158,255,0.95)', 'rgba(150,122,205,0.85)', KEY_GREEN,
          'para se impulsionar mais longe');
      }
    }
  }

  // Checkmarks de dive roll e side flip — posições fixas no mundo
  if (completedFlags.dive) {
    const dcx = 800 - camX;
    if (dcx > -20 && dcx < CANVAS_W + 20) drawCheckmark(dcx, GROUND_Y - 75);
  }
  if (completedFlags.sideFlip) {
    const sfcx = 5400 - camX;
    if (sfcx > -20 && sfcx < CANVAS_W + 20) drawCheckmark(sfcx, GROUND_Y - 155);
  }

  // ── Zona do Drone (x:5550) ─────────────────────────────────────────────
  if (playerX > 4900 && playerX < 6500) {
    const droneHintX = 5550 - camX;
    if (droneHintX > 60 && droneHintX < CANVAS_W - 60) {
      // Linha no chão marcando a zona
      ctx.strokeStyle = 'rgba(255,80,60,0.25)';
      ctx.lineWidth = 1;
      ctx.setLineDash([6, 6]);
      ctx.beginPath();
      ctx.moveTo(droneHintX - 100, GROUND_Y - 1);
      ctx.lineTo(droneHintX + 300, GROUND_Y - 1);
      ctx.stroke();
      ctx.setLineDash([]);
      // Rótulo da zona no chão
      ctx.save();
      ctx.textAlign = 'center';
      ctx.font = '9px monospace';
      ctx.fillStyle = 'rgba(255,100,80,0.4)';
      ctx.fillText('▲ ZONA DO DRONE ▲', droneHintX + 100, GROUND_Y - 5);
      ctx.restore();
      // Balão de dica
      drawHintBalloon(droneHintX, GROUND_Y - 22,
        'SIDE FLIP', 'desvie dos tiros do drone com:', 'ESPAÇO + ESPAÇO',
        'rgba(210,180,255,0.95)', 'rgba(145,130,180,0.85)', KEY_GREEN);
    }
  }

  ctx.restore();

  // Balão fixo no espaço vazio à direita da tela. Usa o mesmo tamanho,
  // peso e cor da linha de comando das outras dicas.
  // Balão tic-tac: só enquanto player ainda não passou para o lado da escada
  if (hasTicTacWall && playerX < 6680) {
    const hintW = 270;
    const hintScreenX = CANVAS_W - hintW / 2 - 16;
    drawHintBalloon(hintScreenX, 130,
      'tic tac', 'pegue impulso em objetos paralelos',
      '→ + ESPAÇO + ← + ESPAÇO',
      'rgba(210,235,255,0.95)', 'rgba(170,210,250,0.9)', KEY_GREEN);
  }
  // Balão da escada: só quando Horácio já caiu do outro lado e está próximo
  if (hasTrainingLadder && playerX > 6680) {
    const hintW = 270;
    const hintScreenX = CANVAS_W - hintW / 2 - 16;
    drawHintBalloon(hintScreenX, 130,
      'Usar escada', 'suba e desça a escada com:', '↑  ↓',
      'rgba(210,235,255,0.95)', 'rgba(170,210,250,0.9)', KEY_GREEN);
  }
}

export function getMusicVolumeSliderRect(): { x: number; y: number; w: number; h: number; trackX: number; trackW: number } {
  const trackW = 90;
  const w = trackW + 100;
  const h = 18;
  const y = 5;
  const x = CANVAS_W - 10 - w;
  const trackX = x + 62;
  return { x, y, w, h, trackX, trackW };
}

export function getSfxCategoryVolumeSliderRect(): { x: number; y: number; w: number; h: number; trackX: number; trackW: number } {
  const trackW = 90;
  const w = trackW + 100;
  const h = 18;
  const y = 27;
...
export function drawEditorUI(
  ...
  isDirty = false,
  musicVolume = 1,
  sfxCategory: { vol: number; label: string } | null = null,
): void {
  ...
  ctx.fillStyle = 'rgba(6,5,15,0.97)';
  ...
  // ── Linha 2: barra de checkpoints ──────────────────────────
  const cpRow2Y = histBtnY + histBtnH + 4;
  const cpRow2H = histBtnH;
  ctx.save();
  ctx.fillStyle = 'rgba(10,12,25,0.78)';
  ctx.beginPath();
  ctx.roundRect(4, cpRow2Y - 1, CANVAS_W - 8, cpRow2H + 2, 3);
  ctx.fill();
  ctx.restore();

  const checkpointBtnX = 8;
  const checkpointBtnY = cpRow2Y;
  const checkpointBtnW = 30;
  const checkpointBtnH = cpRow2H;
  ...
  // ── Botão SALVAR (compacto, mesmo tamanho do UPLOAD) ────────
  {
    const expX = galeriaBtnX + galeriaBtnW + 4;
    const expY = histBtnY;
    const expW = 90;
    const expH = histBtnH;
    ...
    switch (saveStatus) {
      case 'saving':
        bgColor = 'rgba(25,40,75,0.93)';
        strokeColor = `rgba(100,160,${Math.round(255 * pulse)},0.9)`;
        fgColor = 'rgba(150,200,255,0.95)';
        icon = '⏳';
        break;
      case 'pending':
        bgColor = 'rgba(60,50,10,0.93)';
        strokeColor = `rgba(255,${Math.round(200 * pulse)},60,0.9)`;
        fgColor = 'rgba(255,220,130,0.95)';
        icon = '●';
        break;
      case 'error':
        bgColor = 'rgba(75,18,18,0.93)';
        strokeColor = `rgba(255,80,${Math.round(80 * pulse)},0.9)`;
        fgColor = 'rgba(255,160,160,0.98)';
        icon = '⚠';
        break;
      case 'saved':
      default:
        bgColor = 'rgba(15,50,28,0.93)';
        strokeColor = `rgba(50,${Math.round(180 * pulse)},90,0.88)`;
        fgColor = 'rgba(100,240,150,0.95)';
        icon = '✓';
        break;
    }
    ...
    ctx.font = 'bold 10px monospace';
    ctx.textAlign = 'center';
    ctx.fillText(`${icon} SALVAR`, expX + expW / 2, expY + 12);
    ctx.restore();
  }
  ...
  // Botão CTRL: TESTAR AQUI — linha 2, lado direito
  {
    const cpRow2Y = 27;
    const cpRow2H = 18;
    ...
    const btnX = CANVAS_W - 10 - labelW - 2;
    ctx.save();
    ctx.fillStyle = 'rgba(20,55,35,0.92)';
    ctx.strokeStyle = 'rgba(60,210,120,0.7)';
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.roundRect(btnX, cpRow2Y, labelW + 4, cpRow2H, 3);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = 'rgba(80,230,140,0.95)';
    ctx.textAlign = 'right';
    ctx.fillText(spawnLabel, CANVAS_W - 10, cpRow2Y + 12);
    ctx.restore();
  }

  // Barra de volume da música — linha 1, canto direito
  {
    const r = getMusicVolumeSliderRect();
    ctx.save();
    ctx.font = 'bold 10px monospace';
    ctx.textAlign = 'left';
    ctx.fillStyle = 'rgba(220,220,255,0.9)';
    ctx.fillText('🎵 MÚSICA', r.x - 4, r.y + r.h - 5);

    ctx.fillStyle = 'rgba(20,20,35,0.85)';
    ctx.strokeStyle = 'rgba(120,120,170,0.6)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.roundRect(r.trackX, r.y + r.h / 2 - 3, r.trackW, 6, 3);
    ctx.fill();
    ctx.stroke();

    const fillW = Math.max(6, r.trackW * Math.min(1, Math.max(0, musicVolume)));
    ctx.fillStyle = 'rgba(120,200,255,0.9)';
    ctx.beginPath();
    ctx.roundRect(r.trackX, r.y + r.h / 2 - 3, fillW, 6, 3);
    ctx.fill();

    const handleX = r.trackX + fillW;
    ctx.beginPath();
    ctx.fillStyle = 'rgba(220,240,255,1)';
    ctx.strokeStyle = 'rgba(80,160,255,1)';
    ctx.lineWidth = 1.5;
    ctx.arc(handleX, r.y + r.h / 2, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();

    ctx.textAlign = 'left';
    ctx.fillStyle = 'rgba(180,200,255,0.85)';
    ctx.fillText(`${Math.round(Math.min(1, Math.max(0, musicVolume)) * 100)}%`, r.trackX + r.trackW + 8, r.y + r.h - 5);
    ctx.restore();
  }

  // Barra de volume de categoria de objeto — linha 2, canto direito
  {
    const r = getSfxCategoryVolumeSliderRect();
    ctx.save();
    ctx.font = 'bold 10px monospace';
    ctx.textAlign = 'left';
    if (sfxCategory !== null) {
      ctx.fillStyle = 'rgba(255,210,150,0.95)';
      ctx.fillText(`🔊 ${sfxCategory.label}`, r.x - 4, r.y + r.h - 5);
      ...
      const fillW = Math.max(6, r.trackW * Math.min(1, Math.max(0, sfxCategory.vol)));
      ...
      ctx.strokeStyle = 'rgba(255,140,40,1)';
      ctx.lineWidth = 1.5;
      ctx.arc(handleX, r.y + r.h / 2, 6, 0, Math.PI * 2);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.fillText(`${Math.round(Math.min(1, Math.max(0, sfxCategory.vol)) * 100)}%`, r.trackX + r.trackW + 8, r.y + r.h - 5);
    } else {
      ctx.fillStyle = 'rgba(140,140,160,0.45)';
      ctx.fillText('🔊 SFX', r.x - 4, r.y + r.h - 5);
      ctx.fillStyle = 'rgba(20,20,35,0.50)';
      ctx.strokeStyle = 'rgba(120,120,170,0.3)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.roundRect(r.trackX, r.y + r.h / 2 - 3, r.trackW, 6, 3);
      ctx.fill();
      ...
      ctx.fillText('selecione objeto', r.trackX + 2, r.y + r.h - 5);
    }
    ctx.restore();
  }
...
export function drawVictoryScreen(ctx: CanvasRenderingContext2D, score: number, time: number): void {
  ...
  ctx.fillStyle = 'rgba(18,22,18,0.99)';
  ctx.fillRect(cx - 220, cy - 145, 440, 295);
  ctx.strokeStyle = 'rgba(60,190,100,0.65)';
  ctx.lineWidth = 1;
  ctx.strokeRect(cx - 220, cy - 145, 440, 295);
  ctx.fillStyle = 'rgba(60,190,100,0.55)';
  ...
  ctx.font = '12px monospace';
  ctx.fillText(`DISTÂNCIA: ${Math.floor(score / 10)}m`, cx, cy - 48);
...
export function drawGameOverScreen(ctx: CanvasRenderingContext2D, score: number, time: number): void {
  ...
  ctx.font = 'bold 13px monospace';
  ctx.fillText('[ ESPAÇO — MENU INICIAL ]', cx, cy + 92);
...
// ── CASA POPULAR APÓS A ESCADARIA (x:31700 – x:32300) ──────────────────────
// Laje de concreto (y=100) com parapeitos, caixa d'água, chaminé e antena.
// Horácio cai da escadaria e pousa na laje.
const HOUSE_X1    = 31700;
const HOUSE_W     = 600;
const HOUSE_ROOF_Y = -434;
const HOUSE2_X1   = 32500;

export function drawHouseAfterStaircase(ctx: CanvasRenderingContext2D, camX: number, houseX: number = HOUSE_X1, customRoofY?: number): void {
  const sx = houseX - camX;
  if (sx + HOUSE_W < -60 || sx > CANVAS_W + 60) return;

  const roofY  = customRoofY ?? HOUSE_ROOF_Y;
  const houseH = GROUND_Y - roofY;

  ctx.save();

  // ── Corpo do prédio ──────────────────────────────────────────────────────
  const bodyGrad = ctx.createLinearGradient(sx, roofY, sx + HOUSE_W, roofY);
  bodyGrad.addColorStop(0,    '#1e1810');
  bodyGrad.addColorStop(0.35, '#2c2016');
  bodyGrad.addColorStop(0.65, '#241c12');
  bodyGrad.addColorStop(1,    '#18120c');
  ctx.fillStyle = bodyGrad;
  ctx.fillRect(sx, roofY, HOUSE_W, houseH);

  const lGrad = ctx.createLinearGradient(sx, 0, sx + 22, 0);
  lGrad.addColorStop(0, 'rgba(0,0,0,0.65)');
  lGrad.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = lGrad;
  ctx.fillRect(sx, roofY, 22, houseH);

  const rGrad = ctx.createLinearGradient(sx + HOUSE_W - 22, 0, sx + HOUSE_W, 0);
  rGrad.addColorStop(0, 'rgba(0,0,0,0)');
  rGrad.addColorStop(1, 'rgba(0,0,0,0.65)');
  ctx.fillStyle = rGrad;
  ctx.fillRect(sx + HOUSE_W - 22, roofY, 22, houseH);

  ctx.strokeStyle = 'rgba(0,0,0,0.20)';
  ctx.lineWidth = 1;
  for (let hy = roofY + 16; hy < GROUND_Y; hy += 18) {
    ctx.beginPath();
    ctx.moveTo(sx, hy);
    ctx.lineTo(sx + HOUSE_W, hy);
    ctx.stroke();
  }

  // ── Janelas ──────────────────────────────────────────────────────────────
  const winW = 156, winH = 186, winSpX = 180, winSpY = 282;
  const colCount = Math.floor((HOUSE_W - 24) / winSpX);
  const rowCount = Math.floor((houseH - 100) / winSpY);
  for (let col = 0; col < colCount; col++) {
    const wx = sx + 24 + col * winSpX;
    for (let row = 0; row < rowCount; row++) {
      const wy = roofY + 30 + row * winSpY;
      if (wy + winH > GROUND_Y - 90) continue;
      const seed = (col * 11 + row * 7) % 19;
      ctx.fillStyle = seed < 4 ? 'rgba(150,65,15,0.28)' : 'rgba(4,2,1,0.94)';
      ctx.fillRect(wx, wy, winW, winH);
      ctx.strokeStyle = 'rgba(45,28,12,0.85)';
      ctx.lineWidth = 1;
      ctx.strokeRect(wx, wy, winW, winH);
      ctx.strokeStyle = 'rgba(25,15,6,0.6)';
      ctx.beginPath();
      ctx.moveTo(wx + winW / 2, wy + 2);
      ctx.lineTo(wx + winW / 2, wy + winH - 2);
      ctx.stroke();
    }
  }

  // ── Janelas do térreo (3 janelas na faixa inferior) ─────────────────────
  const lwinW = 76, lwinH = 74;
  const lwinY = GROUND_Y - 164;
  const lwinPositions = [sx + 44, sx + 156, sx + 418];
  for (const lx of lwinPositions) {
    // moldura externa
    ctx.fillStyle = 'rgba(18,12,6,0.92)';
    ctx.fillRect(lx - 4, lwinY - 4, lwinW + 8, lwinH + 8);
    // vidro
    const lseed = Math.round((lx - sx) / 10) % 5;
    ctx.fillStyle = lseed < 1 ? 'rgba(140,55,10,0.26)' : 'rgba(4,2,1,0.93)';
    ctx.fillRect(lx, lwinY, lwinW, lwinH);
    // caixilho vertical
    ctx.strokeStyle = 'rgba(35,20,8,0.80)';
    ctx.lineWidth = 2;
    ctx.strokeRect(lx, lwinY, lwinW, lwinH);
    ctx.beginPath();
    ctx.moveTo(lx + lwinW / 2, lwinY + 3);
    ctx.lineTo(lx + lwinW / 2, lwinY + lwinH - 3);
    ctx.stroke();
    // caixilho horizontal
    ctx.beginPath();
    ctx.moveTo(lx + 3, lwinY + lwinH / 2);
    ctx.lineTo(lx + lwinW - 3, lwinY + lwinH / 2);
    ctx.stroke();
    // pingadeira
    ctx.fillStyle = 'rgba(8,5,2,0.80)';
    ctx.fillRect(lx - 5, lwinY - 6, lwinW + 10, 5);
  }

  // ── Porta ────────────────────────────────────────────────────────────────
  const doorW = 52, doorH = 108;
  const doorX = sx + HOUSE_W / 2 - doorW / 2;
  ...
  // painel superior da porta
  ctx.fillStyle = 'rgba(28,18,8,0.80)';
  ctx.fillRect(doorX + 6, doorY + 8, doorW - 12, 28);
  // painel inferior da porta
  ctx.fillStyle = 'rgba(28,18,8,0.80)';
  ctx.fillRect(doorX + 6, doorY + 46, doorW - 12, doorH - 54);
  // maçaneta
  ctx.fillStyle = '#3a2c18';
  ctx.beginPath();
  ctx.arc(doorX + doorW - 10, doorY + doorH * 0.52, 4, 0, Math.PI * 2);
  ctx.fill();
  // verga
  ctx.fillStyle = '#221810';
  ctx.fillRect(doorX - 4, doorY - 6, doorW + 8, 8);

  // ── Laje (telhado plano / terraço) ───────────────────────────────────────
  ctx.fillStyle = '#28261e';
  ctx.fillRect(sx, roofY, HOUSE_W, 18);
  ctx.fillStyle = '#363228';
  ctx.fillRect(sx, roofY, HOUSE_W, 3);
  const lajeShadow = ctx.createLinearGradient(0, roofY + 18, 0, roofY + 35);
  lajeShadow.addColorStop(0, 'rgba(0,0,0,0.50)');
  lajeShadow.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = lajeShadow;
  ctx.fillRect(sx, roofY + 18, HOUSE_W, 17);
  // rachaduras
  ctx.strokeStyle = 'rgba(0,0,0,0.38)';
  ctx.lineWidth = 1;
  ctx.beginPath(); ctx.moveTo(sx + 80, roofY + 4); ctx.lineTo(sx + 112, roofY + 15); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(sx + 380, roofY + 3); ctx.lineTo(sx + 408, roofY + 16); ctx.stroke();

  // ── Parapeitos ───────────────────────────────────────────────────────────
  ctx.fillStyle = '#222018';
  ctx.fillRect(sx, roofY - 28, 14, 28);
  ctx.fillStyle = '#2e2c24';
  ctx.fillRect(sx, roofY - 28, 14, 3);

  ctx.fillStyle = '#222018';
  ctx.fillRect(sx + HOUSE_W - 14, roofY - 28, 14, 28);
  ctx.fillStyle = '#2e2c24';
  ctx.fillRect(sx + HOUSE_W - 14, roofY - 28, 14, 3);


  // ── Chaminé ──────────────────────────────────────────────────────────────
  const chX = sx + 440, chY = roofY - 82, chW = 35, chH = 82;
  ctx.fillStyle = '#2a1a10';
  ctx.fillRect(chX, chY, chW, chH);
  ctx.strokeStyle = 'rgba(0,0,0,0.30)';
  ctx.lineWidth = 1;
  for (let by = chY + 8; by < roofY; by += 10) {
    ctx.beginPath(); ctx.moveTo(chX, by); ctx.lineTo(chX + chW, by); ctx.stroke();
  }
  ctx.fillStyle = '#1e1008';
  ctx.fillRect(chX - 4, chY, chW + 8, 7);
  ctx.fillStyle = '#2c1a10';
  ctx.fillRect(chX - 4, chY, chW + 8, 3);

  // ── Antena ───────────────────────────────────────────────────────────────
  const anX = sx + 510;
  ctx.strokeStyle = '#1a1816';
  ctx.lineWidth = 2;
  ctx.beginPath(); ctx.moveTo(anX, roofY); ctx.lineTo(anX, roofY - 88); ctx.stroke();
  ctx.lineWidth = 1;
  ctx.strokeStyle = '#252320';
  for (let ai = 0; ai < 5; ai++) {
    const ay = roofY - 18 - ai * 13;
    const aw = 22 - ai * 3;
    ctx.beginPath(); ctx.moveTo(anX - aw, ay); ctx.lineTo(anX + aw, ay); ctx.stroke();
  }

  // ── Grafite ──────────────────────────────────────────────────────────────
  ctx.textAlign = 'left';
  ctx.font = 'bold 22px monospace';
  ctx.fillStyle = 'rgba(190,30,18,0.38)';
  ctx.fillText('BRAVUNA', sx + 55, roofY + 145);
  ctx.font = '13px monospace';
  ctx.fillStyle = 'rgba(210,160,55,0.28)';
  ctx.fillText('ORDEM', sx + 370, roofY + 210);

  ctx.restore();
}

// ── Muro com envergadura anti-escalada (x:36336) ──────────────────────────
const BWALL_X     = 36336;
const BWALL_Y     = 289;
const BWALL_W     = 28;
const BWALL_H     = GROUND_Y - BWALL_Y; // 121
const BOVERHANG_W = 210;
const BOVERHANG_H = 30;

// ── Prédio final — Horácio some pela porta ──────────────────────────────
const ENDING_BLDG_X      = 36620;
const ENDING_DOOR_OFFSET = 20;   // faixa de parede à esquerda antes da porta
const ENDING_DOOR_W      = 58;   // largura da abertura (≥ PLAYER_W=26 + margem)
const ENDING_DOOR_H      = 120;  // altura da abertura a partir do chão

export function drawEndingBuilding(ctx: CanvasRenderingContext2D, camX: number): void {
  const sx        = ENDING_BLDG_X - camX;
  const doorX     = sx + ENDING_DOOR_OFFSET;
  const doorRightX = doorX + ENDING_DOOR_W;
  const farRightW = 1200;

  if (doorRightX + farRightW < -60 || sx > CANVAS_W + 60) return;

  const floorY   = GROUND_Y;
  const doorTopY = floorY - ENDING_DOOR_H;
  ...
  function fillBricks(bx: number, bw: number) {
    if (bw <= 0) return;
    ctx.fillStyle = '#1c1712';
    ctx.fillRect(bx, 0, bw, floorY);
    for (let row = 0; row * ROW_H < floorY; row++) {
      const ry  = row * ROW_H;
      const off = (row % 2) * Math.floor(BRICK_W / 2);
      ctx.fillStyle = 'rgba(0,0,0,0.35)';
      ctx.fillRect(bx, ry + BRICK_H, bw, MORTAR);
      for (let brickX = bx - off; brickX < bx + bw; brickX += BRICK_W) {
        const bl = Math.max(brickX, bx);
        const br = Math.min(brickX + BRICK_W - 1, bx + bw);
        if (br <= bl) continue;
        ctx.fillStyle = 'rgba(55,44,30,0.28)';
        ctx.fillRect(bl, ry, br - bl, 2);
        if (brickX > bx) {
          ctx.fillStyle = 'rgba(0,0,0,0.38)';
          ctx.fillRect(bl, ry, 1, BRICK_H);
        }
      }
    }
    // sombra lateral esquerda
    const lg = ctx.createLinearGradient(bx, 0, bx + Math.min(18, bw), 0);
    lg.addColorStop(0, 'rgba(0,0,0,0.65)');
    lg.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = lg;
    ctx.fillRect(bx, 0, Math.min(18, bw), floorY);
  }

  ctx.save();

  // Seção esquerda da parede
  fillBricks(sx, ENDING_DOOR_OFFSET);

  // Seção direita (vai até o fim do mundo visível)
  fillBricks(doorRightX, farRightW);

  // Interior escuro da porta
  ctx.fillStyle = '#000';
  ctx.fillRect(doorX, doorTopY, ENDING_DOOR_W, ENDING_DOOR_H);

  // Brilho quente vindo de dentro (luz ambiente do interior)
  const gg = ctx.createLinearGradient(doorX, floorY, doorX, doorTopY);
  gg.addColorStop(0, 'rgba(255,155,55,0.12)');
  gg.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = gg;
  ctx.fillRect(doorX, doorTopY, ENDING_DOOR_W, ENDING_DOOR_H);

  // Verga de concreto (topo da porta)
  ctx.fillStyle = '#2a2018';
  ctx.fillRect(doorX - 4, doorTopY - 8, ENDING_DOOR_W + 8, 10);
  ctx.fillStyle = '#0a0806';
  ctx.fillRect(doorX - 4, doorTopY - 8, ENDING_DOOR_W + 8, 2);

  // Ombreiras verticais
  ctx.fillStyle = '#161210';
  ctx.fillRect(doorX - 3, doorTopY, 4, ENDING_DOOR_H);
  ctx.fillRect(doorRightX - 1, doorTopY, 4, ENDING_DOOR_H);

  // Friso superior do prédio
  ctx.fillStyle = '#0a0806';
  ctx.fillRect(sx, 0, ENDING_DOOR_OFFSET, 4);
  ctx.fillRect(doorRightX, 0, farRightW, 4);

  ctx.restore();
}

export function drawBlockingWall(
  ctx: CanvasRenderingContext2D,
  camX: number,
  wx = BWALL_X,
  wy = BWALL_Y,
  ...
  const sx = wx - camX;
  if (sx + ww + BOVERHANG_W < -60 || sx > CANVAS_W + 60) return;
  ...
  function drawBrickWall(
    ...
    brickW: number, baseColor: string, lightColor: string
  ) {
    ctx.fillStyle = baseColor;
    ctx.fillRect(x, y, w, h);
    for (let row = 0; y + row * ROW_H < y + h; row++) {
      const ry = y + row * ROW_H;
      const offset = (row % 2) * (brickW / 2);
      // linhas de argamassa horizontal
      ctx.fillStyle = 'rgba(0,0,0,0.38)';
      ctx.fillRect(x, ry + BRICK_H, w, MORTAR);
      // tijolos individuais: lado esquerdo claro + direito escuro + divisores verticais
      for (let bx = x - offset; bx < x + w; bx += brickW) {
        const bLeft  = Math.max(bx, x);
        const bRight = Math.min(bx + brickW - 1, x + w);
        if (bRight <= bLeft) continue;
        // topo do tijolo (highlight)
        ctx.fillStyle = lightColor;
        ctx.fillRect(bLeft, ry, bRight - bLeft, 2);
        // divisor vertical (argamassa)
        if (bx > x) {
          ctx.fillStyle = 'rgba(0,0,0,0.42)';
          ctx.fillRect(bLeft, ry, 1, BRICK_H);
        }
      }
    }
  }

  // ── Corpo do muro (vertical) — usa dimensões reais da plataforma ─────
  drawBrickWall(sx, wy, ww, wh, ww + 1, '#23201a', 'rgba(80,68,50,0.35)');

  // sombra lateral esquerda
  const lsh = ctx.createLinearGradient(sx, 0, sx + 9, 0);
  lsh.addColorStop(0, 'rgba(0,0,0,0.60)');
  lsh.addColorStop(1, 'rgba(0,0,0,0)');
  ctx.fillStyle = lsh;
  ctx.fillRect(sx, wy, 9, wh);

  // ── Topo do muro: bordão de concreto ────────────────────────────────
  ctx.fillStyle = '#0f0c08';
  ctx.fillRect(sx - 2, wy - 4, ww + 4, 6);
