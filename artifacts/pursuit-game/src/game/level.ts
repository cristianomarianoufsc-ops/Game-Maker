...
export function generateLevel(): Platform[] {
  ...
  const groundSegments: Array<{ x: number; w: number }> = [
    ...
    { x: RIVER2.X2, w: 32000 - RIVER2.X2 },        // 30450 → 32000 (margem direita do rio 2 + área da escada)
    { x: 32000, w: 5600 },                          // 32000 → 37600 (prédios pós-escadaria + pneus amortecedores + fuga final)
  ];
  ...
  cans.filter(({ x }) => !isNearWallBase(x, CAN_W)).forEach(({ x }) => {
    platforms.push({ x, y: GROUND_Y - CAN_H, w: CAN_W, h: CAN_H, type: 'obstacle', vaultTrigger: x === 3400 });
  });

  // ── OBSTÁCULO KONG VAULT (x:18972) ───────────────────────────────
  platforms.push({ x: 18972, y: GROUND_Y - 102, w: 239, h: 102, type: 'obstacle', vaultTrigger: true, vaultFrontOnly: true });
  ...
  junkyardItems.filter(({ x, w }) => !isNearWallBase(x, w)).forEach(({ x, type, w, h, collisionW: customCollisionW, collisionH: customCollisionH, collisionOffsetX: customCollisionOffsetX, collisionOffsetY: customCollisionOffsetY, collisionBoxes, cropLeft, cropTop, cropRight, cropBottom, yOffset }) => {
    if (type === 'car') {
      if (collisionBoxes && collisionBoxes.length > 0) {
        ...
        platforms.push({ x, y, w, h, type, collisionBoxes, cropLeft, cropTop, cropRight, cropBottom, vaultTrigger: x === 12795 });
      } else {
  ...
  const junkyardBoxStacks: Platform[] = [
    ...
    { x: 12570, y: GY - 385,            w: BOX_W, h: BOX_H, type: 'box' },
    { x: 12570, y: GY - 440,            w: BOX_W, h: BOX_H, type: 'box' },
    { x: 12570, y: GY - 495,            w: BOX_W, h: BOX_H, type: 'box' },
    ...
    { x: 14107, y: GY - 275,           w: BOX_W, h: BOX_H, type: 'box' },
    { x: 14172, y: GY - 275,           w: BOX_W, h: BOX_H, type: 'box' },
    { x: 14237, y: GY - 275,           w: BOX_W, h: BOX_H, type: 'box' },

    // Pilha x:21635 (junto ao muro do ferro velho, lado esquerdo)
    { x: 21635, y: GY - BOX_H,         w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - BOX_H * 2,     w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - BOX_H * 3,     w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - BOX_H * 4,     w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - 275,           w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - 330,           w: BOX_W, h: BOX_H, type: 'box' },
    { x: 21635, y: GY - 385,           w: BOX_W, h: BOX_H, type: 'box' },
  ...
  platforms.push({
    ...
    vaultTrigger: true,
    vaultFrontOnly: true, // kong vault só pela frente; pulo do topo é livre
  });
  platforms.push({
    x: 17407,
    y: GROUND_Y - 111,
    ...
    // sem collisionBoxes: sprite decorativo, não bloqueia Horácio
  });
  ...
  // ── REGIÃO FINAL (x:29000–36600) — escadaria, prédios, pneus amortecedores, muros ──
  // Paredes escaláveis na área da escadaria
  platforms.push({ x: 29540, y: GROUND_Y - 202, w: 20, h: 202, type: 'wall', climbable: true, lowJumpImpulse: true });
  platforms.push({ x: 30578, y: GROUND_Y - 179, w: 20, h: 179, type: 'wall', climbable: true });

  // Escadaria em zigue-zague (rampas invisíveis — render feito por drawStaircase)
  platforms.push({ x: 31074, y: GROUND_Y - 288, w: 400, h: 144, type: 'platform', hideRender: true, noAutoRoll: true, slopeRunDown: true, slopeTop: { left: 144, right: 0 }, _stair: true, collisionBoxes: [{ x: 12, y: 0, w: 357, h: 144, slopeTop: { left: 141, right: 0 } }], flipX: true } as Platform);
  platforms.push({ x: 31199, y: GROUND_Y - 144, w: 400, h: 144, type: 'platform', hideRender: true, noAutoRoll: true, slopeRunDown: true, slopeTop: { left: 144, right: 0 }, _stair: true, collisionBoxes: [{ x: 12, y: 0, w: 357, h: 144, slopeTop: { left: 141, right: 0 } }], flipX: false } as Platform);
  platforms.push({ x: 31199, y: GROUND_Y - 432, w: 400, h: 144, type: 'platform', hideRender: true, noAutoRoll: true, slopeRunDown: true, slopeTop: { left: 144, right: 0 }, _stair: true, collisionBoxes: [{ x: 12, y: 0, w: 357, h: 144, slopeTop: { left: 141, right: 0 } }], flipX: false } as Platform);
  platforms.push({ x: 31074, y: GROUND_Y - 576, w: 400, h: 144, type: 'platform', hideRender: true, noAutoRoll: true, slopeRunDown: true, slopeTop: { left: 144, right: 0 }, _stair: true, collisionBoxes: [{ x: 12, y: 0, w: 357, h: 144, slopeTop: { left: 141, right: 0 } }], flipX: true } as Platform);
  platforms.push({ x: 31276, y: GROUND_Y - 720, w: 400, h: 144, type: 'platform', hideRender: true, noAutoRoll: true, slopeRunDown: true, slopeTop: { left: 144, right: 0 }, _stair: true, collisionBoxes: [{ x: 12, y: 0, w: 357, h: 144, slopeTop: { left: 141, right: 0 } }], flipX: false } as Platform);

  // Muro lateral do 1º prédio pós-escadaria + telhados (oneWay — Horácio cai e pousa)
  // NÃO climbable: jogador não pode escalar nem fazer wall-run neste muro.
  // Ghost usa gravação (player-recording.json) para transpor este obstáculo.
  platforms.push({ x: 31676, y: GROUND_Y - 843, w: 20, h: 844, type: 'wall' } as Platform);
  platforms.push({ x: 31700, y: GROUND_Y - 844, w: 600, h: 18, type: 'platform', hideRender: true, oneWay: true });
  platforms.push({ x: 31870, y: GROUND_Y - 914, w: 80, h: 70, type: 'platform', hideRender: true });
  platforms.push({ x: 32500, y: GROUND_Y - 843, w: 597, h: 18, type: 'platform', hideRender: true, oneWay: true });
  platforms.push({ x: 33297, y: GROUND_Y - 843, w: 597, h: 18, type: 'platform', hideRender: true, oneWay: true });
  platforms.push({ x: 34274, y: GROUND_Y - 563, w: 597, h: 18, type: 'platform', hideRender: true, oneWay: true });
  platforms.push({ x: 35500, y: GROUND_Y - 844, w: 597, h: 18, type: 'platform', hideRender: true, oneWay: true });

  // Pilhas de pneus amortecedores — Horácio cai do telhado e pousa aqui
  // cushionOnLand: absorve a queda; oneWay: pode pular de dentro pra cima
  platforms.push({ x: 34920, y: GROUND_Y - 117, w: 90, h: 117, type: 'tireHideout', cushionOnLand: true, oneWay: true } as Platform);
  platforms.push({ x: 35030, y: GROUND_Y - 117, w: 90, h: 117, type: 'tireHideout', cushionOnLand: true, oneWay: true } as Platform);
  platforms.push({ x: 35140, y: GROUND_Y - 117, w: 90, h: 117, type: 'tireHideout', cushionOnLand: true, oneWay: true } as Platform);
  platforms.push({ x: 35250, y: GROUND_Y - 117, w: 90, h: 117, type: 'tireHideout', cushionOnLand: true, oneWay: true } as Platform);
  platforms.push({ x: 35360, y: GROUND_Y - 117, w: 90, h: 117, type: 'tireHideout', cushionOnLand: true, oneWay: true } as Platform);

  // Muro final — hideRender: drawPlatforms pula; só drawBlockingWall renderiza
...
// ── Sala de treino — acessível pelo painel de Opções ─────────────────────────
// Layout: espaço largo para correr → muro baixo → obstáculo de kong vault → muro climbable
export function generateTrainingLevel(): Platform[] {
  return [
    // Chão — largo o suficiente para correr e pegar distância para o kong vault
    { x: -300, y: GROUND_Y, w: 24200, h: GH, type: 'ground' },
    // Caixa baixa — introdução ao pulo básico (ESPAÇO)
    { x: 200, y: GROUND_Y - 48, w: 62, h: 48, type: 'wall' },
    // [espaço para treinar dive roll]
    // Muro climbable antecipado — treino de wall-run, wall-climb e wall-flip
    { x: 2176, y: GROUND_Y - 210, w: 20, h: 210, type: 'wall', climbable: true },
    // Obstáculo para treino de monkey vault
    { x: 2700, y: GROUND_Y - 102, w: 239, h: 102, type: 'obstacle', vaultTrigger: true },
    // Obstáculo para treino de kong vault (dive roll + espaço ao encostar)
    { x: 3800, y: GROUND_Y - 102, w: 239, h: 102, type: 'obstacle', vaultTrigger: true, isKongVault: true },
    // Bloco baixo — treino de rolamento por baixo (h:62 visual, hitbox h:85 = 40px de folga até o chão)
    { x: 4715, y: GROUND_Y - 125, w: 115, h: 62, type: 'platform', isRollUnder: true,
      collisionBoxes: [{ x: 0, y: 0, w: 115, h: 85 }],
    } as import('./types').Platform,
    // Tic-tac no fim da sala, depois da zona dos tiros do drone.
    // Mantém a mesma geometria do trecho final da história:
    // parede de impulso x:36144 → parede paralela x:36321 (177px).
    { x: 6225, y: GROUND_Y - 382, w: 25, h: 231, type: 'wall',
      climbable: true, noClimbOver: true, tictacWall: true },
    { x: 6402, y: GROUND_Y - 289, w: 25, h: 289, type: 'wall',
      climbable: true, noHang: true },
    // Último exercício: escada curta, treino de subida rápida
    { x: 6785, y: GROUND_Y - 276, w: 60, h: 276, type: 'wall',
      isLadder: true, hideRender: true },
    // Parede de fim da sala, encostada na lateral direita da escada.
    { x: 6845, y: -1600, w: 1600, h: GROUND_Y + 1600, type: 'wall', noHint: true, endWall: true },
  ];
}
